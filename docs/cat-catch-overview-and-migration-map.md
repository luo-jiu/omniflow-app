...
当前固定目标为 Cat Catch `2cb981d7c2f4614732edccc167c4b5793d1cb138`。当前映射包含 7 个 unit 和 32 项 capability，226 个唯一 active 测试引用、229 个计划测试 ID，均以 capability map 为准。`network-capture`、`deep-search-runtime` 与 `hls-engine` 已全部验证并完成原子 cutover；Deep 的 production document factory 现在直接组合 stateful discovery、experience hooks、inline/Vimeo page helper、page-origin toolkit state、generated-resource owner 和 tokenized main ingress。旧 disabled hooks、manifest heuristic、Worker bootstrap、toolkit state、`probeResources` 及混合 host 已删除；11 个平台适配保留，17 个 `remove-after-cutover` 条目作为删除门禁继续由 validator 检查。MSE 已形成 page/spool 唯一 owner，新增默认关闭的 `saveEveryGigabyte` 周期保存链路（flush 后按所有轨道累计 1 GiB 阈值串行输出，成功后清理 page cache/spool），并补齐可选捕获开关下的完成标志、“无轨道但已完成”清理边界、稀疏缓冲首段自动跳转、flush 跨次以及最终 drain 的头部裁剪；merge/transcode 失败会清理 partial output，ffmpeg 零退出也必须留下非空 output；条件式真实 FFmpeg/FFprobe fixture 已验证独立 fMP4 音视频轨道合并为可探测的 H.264/AAC MP4，但固定上游 parity、生产大媒体、完整提取/导入和真实网站验证仍开放；DASH parser、`DashTaskExecutor`、`DashLiveTask`、main XML adapter、增量追加 helper、独立 session owner 和 output adapter 已建立处理基座并接入现有 MPD 分析与生产 dispatch，Period/AdaptationSet/Representation SegmentTemplate、SegmentList、SegmentBase 继承、SegmentBase SIDX expansion、SegmentTemplate `endNumber`、SegmentTemplate dynamic client clock offset、SegmentList timeline、静态最后一片 duration 修正、从下一个显式 start/MPD 总时长推导的静态 Period 时长、同身份静态多 Period 串接、具备 availability 证据的动态有限 `r=-1` 展开和动态 duration-only 模板窗口已加入；带有限当前窗口的 dynamic snapshot 现可执行，live start/stop/discard main/preload IPC 已接入但当前没有 renderer UI，且 live MPD 必须与 captured resource authority 精确同 URL，轮询终止错误会清理 live session/workdir 并保留最后错误 snapshot，静态双轨和动态刷新追加的真实 FFmpeg/FFprobe 输出证据已加入；SegmentBase 已支持受限深度的嵌套 SIDX fetch，超过深度会显式拒绝，不完整或初始化冲突的多 Period 集合仍开放；普通直链、已捕获资源和 HLS 计划下载已接入 `StagedOutputLeaseStore`/`publishStagedOutput`，其余 output integration 仍开放。其余 unit 仍开放；data/blob、未捕获派生 URL 等明确平台 fallback 不代表第二套已切换 owner 仍在运行。

截至 2026-08-29，capability map 的最新统计为 226 个 active 测试引用、229 个计划测试 ID、104 个 cleanup entry；普通直链、已捕获资源、HLS/MPD 直拉、HLS 计划下载、独立轨合并、HLS 直播停止导出、DASH 静态计划、DASH 直播停止导出和手动 MSE 合并已接入 staged output lease，但不改变其他 output 能力仍在迁移中的结论。
...
已捕获资源转码同样经过 main-only staged lease，处理失败不会覆盖用户已有目标；它仍使用现有 ffmpeg executor，跨入口取消和最终资料库交付尚未统一。

MSE 手动音视频合并现在也经过 staged lease；MSE 自动完成和周期保存仍沿用 download staging root + download/import 事件链，后续需要单独收口其取消、配额和交付终态。
