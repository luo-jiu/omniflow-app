...
当前固定目标为 Cat Catch `2cb981d7c2f4614732edccc167c4b5793d1cb138`。初始映射包含 7 个 unit 和 32 项 capability；其中 network classifier/rules 为 `ported-unverified`，其余 31 项仍为 `pending`。目前只有 2 个计划测试 ID 已落成真实 pure behavior test，尚未建立 production-equivalent integration 或完成任何 unit cutover。
