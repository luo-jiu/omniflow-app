...
当前固定目标为 Cat Catch `2cb981d7c2f4614732edccc167c4b5793d1cb138`。初始映射包含 7 个 unit 和 32 项 capability；逐项状态以 capability map 为准，目前有 124 个唯一计划测试引用已落成真实 pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚未完成任何 unit cutover。目标 network runtime、opaque resource authority、部分 HLS/DASH transport 与 HLS pure parser/plan 已进入 production 或等价测试链；HLS DTO/parser/plan、local pipeline、local-to-ffmpeg 阶段序列与 live polling 已由 target owner 接管，生产调用方不再依赖三个旧 façade。其中首次计划与失败分片 retry 已复用同一 executor 合同，`tsAddArg` 已按 fragment-only 边界接入普通静态计划、main-owned 独立双轨本地计划和直播 task，分片 retry 的 byte Range/取消边界、keyless encrypted/manual-key fallback、CBC 加密 MAP 的 range 补齐/previous-block IV、无值 `DISCONTINUITY/ENDLIST` 的前缀行为、fast tokenizer 的空格/Tab 行首优先级、AttrList 未引号 value 的空格保留、PART 对 fragment duration 的状态影响、变量替换为空的 segment URI 拒绝和 effective URL authority 已有专项证据；真实输出门禁已覆盖 AES-128、AES-256 CBC/CTR，以及加密 fMP4/H264 视频与独立 AES-128/AAC 音轨的本地双轨合并。完整 parser/processing 语义、legacy-named controller adapter、未捕获 fallback、旧 toolkit 及 unit 原子 cutover 仍待完成，因此旧链仍部分承担生产职责。
