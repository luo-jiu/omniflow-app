...
当前固定目标为 Cat Catch `2cb981d7c2f4614732edccc167c4b5793d1cb138`。初始映射包含 7 个 unit 和 32 项 capability，124 个唯一计划测试引用已落成真实 pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`hls-engine` 的 4 项 capability 已全部验证，唯一 dispatch boundary 已使用 target，3 个 compatibility 文件和 legacy-named handler 已在同一切片删除，因此它是首个完成原子 cutover 的 unit。其余 6 个 unit 仍开放；未捕获派生 HLS URL 走 embedded session 是保留的平台 adapter，不代表旧 HLS 算法仍在运行。
