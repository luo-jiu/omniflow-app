...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；`processing/task-registry.ts#ProcessingTaskRegistry` 维护全局活动任务集合，应用退出由 `main.ts` 调用 registry `dispose()` 等待并终止这些任务。外部命令、native `will-download` session 和普通直接/捕获资源流式下载也已登记到同一 registry；普通直接/捕获、HLS 计划下载、独立轨合并和 HLS 直播停止导出现在通过 `processing/staged-output-publisher.ts#publishStagedOutput` 在 owner-scoped lease 中完成处理，成功 claim 后再发布到用户目标，兼容保留最终 `outputPath`；native 下载由 `processing/native-download-session.ts#NativeDownloadSession` 负责 staging、进度、取消和终态清理。`processing/staged-output-lease.ts#StagedOutputLeaseStore` 已建立 owner-scoped path、单次 claim、TTL touch/reap 和 release 的 main-only 基座。HLS host lifecycle 仍负责入口归属，MSE spool 仍有独立 bounded owner；HLS 直播录制中的 workdir、DASH 直播、DASH 计划、MSE 其他 output、Cat Catch native BrowserWindow 下载 fallback、协议任务、跨入口用户取消协议、local-save/UploadManager handoff 和 crash quarantine 仍待统一，ffmpeg 可执行探测仍是独立短进程。
