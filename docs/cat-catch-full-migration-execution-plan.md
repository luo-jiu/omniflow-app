...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；`processing/task-registry.ts#ProcessingTaskRegistry` 维护全局活动任务集合，应用退出由 `main.ts` 调用 registry `dispose()` 等待并终止这些任务。外部命令也已登记到同一 registry，并在快速 dispatch 返回后继续保留直到真实进程退出。HLS host lifecycle 仍负责入口归属，MSE spool 仍有独立 bounded owner；协议任务、普通下载、跨入口用户取消协议和 staged output lease 仍待登记/统一，ffmpeg 可执行探测仍是独立短进程。
...
因此当前 32 项能力中有 15 项 `verified`、6 项 `porting`、1 项 `ported-unverified`、10 项 `pending`；`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成 production cutover，MSE 仍不能因为局部 target 测试通过就提前关闭，其余 unit 也仍开放。
