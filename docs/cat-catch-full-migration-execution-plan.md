...
- HLS 本地 key/map/segment 下载与 playlist 重写由 `processing/hls-task.ts#HlsTaskExecutor` 持有，直播轮询与累计计划由 `processing/hls-live-task.ts#HlsLiveTask` 持有；controller、双轨合并和 output 路径均直接依赖 target owner。两个旧顶层文件只保留同引用兼容出口，继续作为原子 cutover 的 cleanup sentinel；controller 中的 plan task 编排尚未迁出。
