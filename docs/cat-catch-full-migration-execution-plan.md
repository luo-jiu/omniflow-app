...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；executor 维护全局活动任务集合，应用退出由 `main.ts` 调用 `dispose()` 等待并终止这些任务。HLS host lifecycle 仍负责入口归属，MSE spool 仍有独立 bounded owner；跨入口用户取消协议、staged output lease 和 ffmpeg 可执行探测仍待统一，其他输出入口也尚未全部接入。
