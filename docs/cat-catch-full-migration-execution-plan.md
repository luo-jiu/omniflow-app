...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；HLS host lifecycle 仍负责其入口归属，MSE spool 仍有独立 bounded owner。当前尚无应用级 task registry、跨入口取消协议或 app-exit cleanup owner，ffmpeg 可执行探测仍是独立短进程，其他输出入口也尚未全部接入。
