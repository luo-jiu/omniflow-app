...
- HLS 纯 port 已补上 master 普通 variant/I-frame/未知 codec 过滤与无 level 拒绝、同 identity/同 URI 的重复 variant 合并及完整 AUDIO/SUBTITLES group 集合投影、rendition 投影与 child authority、`EXT-X-SESSION-KEY` 变量解析但不进入下载 key state、媒体分片继承紧邻前序 BYTERANGE end、MAP 独立 range、MAP 前置独立 BYTERANGE 双重绑定及缺失/空 MAP URI 拒绝、full-segment AES media/MAP effective IV、独立 key context、固定 full EME build 的 KEYFORMAT 支持/忽略/继承/多 key 选择、正数 `EXT-X-SKIP` delta 不进入错误下载计划、初始 discontinuity sequence、空/无效媒体清单及重复 singleton 标签拒绝和 PNG/JPEG 伪装分片剔除等首批语义并有 fixture；跨 URI pathway fallback、完整 parser 差分和最终 pipeline cutover 仍待完成。
