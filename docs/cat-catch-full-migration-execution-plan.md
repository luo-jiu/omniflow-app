...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；`processing/task-registry.ts#ProcessingTaskRegistry` 维护全局活动任务集合，应用退出由 `main.ts` 调用 registry `dispose()` 等待并终止这些任务。外部命令和普通直接/捕获资源流式下载也已登记到同一 registry；流式下载在 bounded Transform 中写入临时文件，成功后才发布到用户目标，并在真实退出或取消后释放。HLS host lifecycle 仍负责入口归属，MSE spool 仍有独立 bounded owner；native BrowserWindow 下载 fallback、协议任务、跨入口用户取消协议和 staged output lease 仍待统一，ffmpeg 可执行探测仍是独立短进程。
...
因此当前 32 项能力中有 15 项 `verified`、7 项 `porting`、1 项 `ported-unverified`、9 项 `pending`；`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成 production cutover，MSE 仍不能因为局部 target 测试通过就提前关闭，其余 unit 也仍开放。
