...
- DASH target parser/task 已建立继承 BaseURL、Period/AdaptationSet/Representation segment info、模板 token、SegmentTemplate `endNumber`/SegmentList timeline、SegmentBase SIDX index-range expansion、静态最后一片 duration 修正、有限及 availability-bounded `r=-1`、dynamic duration-only availability window、同身份静态多 Period 串接、SegmentList range、DRM 投影、range-aware transfer、顺序写入、取消和失败清理的纯基座；带有限当前窗口的 dynamic snapshot 已可进入同一下载/合并路径；dynamic task/live refresh、复杂嵌套 SIDX、复杂跨层数组合并、完整 `mpd-parser` 差分、ffmpeg 取消与真实生产输出仍不完整。
...
3. 继续 `dash-engine` parser-planner：同身份静态多 Period 串接、availability-bounded `r=-1`、dynamic duration-only 窗口和有限 dynamic snapshot 下载已加入并由 fixture 锁定；下一步补 dynamic manifest refresh/live 端到端、复杂 SIDX 边界，再为下载/合并补 production-equivalent integration；当前 dispatch 已切换到 target task/output owner，不把现有 legacy 行为当 oracle。
