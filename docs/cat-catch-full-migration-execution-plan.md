...
因此当前 32 项能力中有 15 项 `verified`、12 项 `porting`、1 项 `ported-unverified`、4 项 `pending`；`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成 production cutover，MSE 仍不能因为局部 target 测试通过就提前关闭，其余 unit 也仍开放。
