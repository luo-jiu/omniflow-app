...
2. 按当前 `11 retain / 11 pure remove / 9 split-before-remove` 边界，继续从混合旧 probe 抽出 MSE runtime/action 与通用 console/global API，再在唯一 dispatch boundary 原子切换并删除纯 deep block；生产 hooks 在 cleanup 门禁通过前保持关闭。
