...
- HLS 纯 port 已补上同一资源隐式 BYTERANGE offset、初始 discontinuity sequence 和 PNG/JPEG 伪装分片剔除的第一段语义并有 fixture；pipeline wiring、完整 parser 差分、一次性 manifest cache fallback 等经验分支仍待迁移。
