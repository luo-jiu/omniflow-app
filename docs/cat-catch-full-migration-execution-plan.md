...
2. 把 toolkit state body 组合进已验证的 target-only 完整 probe，随后在唯一 dispatch boundary 原子切换并删除旧 deep block；生产 hooks 在 all-frame、round-trip 与 cleanup 门禁同时通过前保持关闭。
