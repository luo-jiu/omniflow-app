...
- HLS、DASH、MSE merge/transcode 和 media-tool operation 已共享 `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 的可取消进程、进度、process-tree 终止、partial output 清理和非空产物校验合同；`processing/task-registry.ts#ProcessingTaskRegistry` 维护全局活动任务集合，应用退出由 `main.ts` 调用 registry `dispose()` 等待并终止这些任务。HLS host lifecycle 仍负责入口归属，MSE spool 仍有独立 bounded owner；协议任务、普通下载、外部工具、跨入口用户取消协议和 staged output lease 仍待登记/统一，ffmpeg 可执行探测仍是独立短进程。
