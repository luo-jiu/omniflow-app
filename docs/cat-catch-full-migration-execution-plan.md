...
- HLS 本地 key/map/segment 下载、playlist 重写以及 local -> ffmpeg 阶段序列由 `processing/hls-task.ts#HlsTaskExecutor` 持有，首次计划和失败分片 retry 共用这一执行合同；直播轮询与累计计划由 `processing/hls-live-task.ts#HlsLiveTask` 持有。controller 只保留 authority、保存路径、session、IPC response 与产品日志 adapter；两个旧顶层文件和 legacy-named controller handler 继续作为原子 cutover 的 cleanup sentinel。
