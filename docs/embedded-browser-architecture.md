...
- `processing/task-registry.ts#ProcessingTaskRegistry` 是 main-side 活动任务登记与退出等待 owner；`processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 通过它登记每个 ffmpeg 进程，外部命令、native `will-download` session、普通直接/捕获资源流式下载、手动 local-save/MSE resource export 以及自动/周期/工具触发的 MSE output 也在真实终态前保持登记。HLS/DASH 分片 downloader 现在接受单一外部 `AbortSignal`，由 downloader 自身取消所有并发请求；HLS manifest/track、HLS 计划下载、DASH local-file merge、MSE merge、captured-resource transcode 和 media-tool operation 均通过共享 executor 执行，统一处理 `-progress` 输出、AbortSignal、跨平台 process-tree 终止、失败 partial output 清理及零退出后的非空文件校验；普通直接/捕获、手动 local-save/MSE resource export、HLS/MPD 直拉、HLS 计划下载和 DASH 静态计划由 `StreamingTransfer`、`publishEmbeddedBrowserExtractedResource`、共享 manifest runner、`HlsTaskExecutor` 或 `DashTaskExecutor` 写入 owner-scoped `StagedOutputLeaseStore`，通过 `publishStagedOutput` 在处理完成后一次性发布到用户目标，MSE 自动 output 由 `stageMseDownloadResource` 写入 download staging 并在事件交付失败时清理，native 下载通过 `NativeDownloadSession` 负责 staging、进度、取消和终态清理。浏览器初始化时会按 24 小时 TTL 清理 download staging root 中上次运行遗留的文件，但仍保留当前的 renderer/import 事件交付。应用退出时 `main.ts` 调用 registry `dispose()` 等待并终止活动进程；HLS/DASH 直播、DASH 计划的更广泛 registry 归属、MSE 自动 output 的 renderer-unmount recovery、Cat Catch native BrowserWindow 下载 fallback、分片任务 registry 归属、协议任务、跨入口用户取消、UploadManager handoff/delivery terminal 和 crash quarantine 仍待接入。
...
- MSE 深度捕捉当前已补第一版增量写盘：`cat-catch-port/mse/runtime.ts` 在 `appendBuffer` 累积超过阈值后由 `capture/adapters/mse-page.ts` 通过 tokenized probe relay 把 chunk flush 给 main，`processing/mse-spool.ts#MseSpoolStore` 以 per-track queue、单轨/总量预算和 TTL 落到 temp spool file；后续 `save / merge / transcode` 以及 MSE 下载动作优先逐轨读取 file-backed 资源，而不是再次整段 base64 提取。自动下载会在 main 侧优先合并音视频，使用 tab-scoped `mse-download` task 和共享 AbortSignal，随后通过 `processing/mse-download-output.ts#stageMseDownloadResource` 写入现有 embedded-browser download staging root；写入、取消或完成事件交付失败时清理 owned file，成功后发布完成事件。`src/features/embedded-browser/workflows/captured-output-workflow-coordinator.ts#CapturedOutputWorkflowCoordinator` 在 renderer 侧持有应用级完成队列、`pending/importing/saving` 交付状态和单一下载订阅，library-detail listener 卸载/重挂载不会丢失待导入文件或正在进行的 UploadManager batch；失败/取消交付回到 pending，只有成功终态才清理临时文件。页面导航、view/process 销毁、tab close 和 controller dispose 会清理 spool。浏览器初始化还会回收超过 24 小时的 download staging 遗留文件。条件式真实 fragmented-MP4 双轨 ffmpeg/ffprobe fixture 已验证 output merge，失败会清理 partial output，零退出也要求非空文件，flush 与最终 drain 也统一了初始化头裁剪；固定上游 parity、生产大媒体、真实页面、library target 冻结、crash quarantine 和完整应用重启恢复仍待验证。
