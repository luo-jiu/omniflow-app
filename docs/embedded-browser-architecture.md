...
- 工具区当前承接两条 HLS 主线：
  - 网络 manifest：继续走 `ffmpeg` 直拉，但 direct 只接受 renderer 从当前 active resource snapshot 找到的精确 opaque `resourceId`；独立音视频轨分别提交 `videoResourceId / audioResourceId`，main 独立兑换两个 URL 和 protected header 集，再把 header 分别绑定到对应 ffmpeg 输入。所选 variant/rendition 尚未被当前页面捕捉时会在启动保存/ffmpeg 前明确拒绝，不把父 manifest 的权限复用于任意子 URL。
...
- HLS manifest 解析的 main-owned inspection，以及 live recorder 的首次 manifest 请求，在普通 HTTP 响应失败时会按 Cat Catch 经验分支仅再请求一次同 URL，并设置 `cache: force-cache`；该回退继续使用 opaque resource authority 恢复 main-owned headers，不接受 renderer 自定义凭据。无响应的网络异常、MPD、分片和后续直播轮询都不会触发这一分支。
...
- Cat Catch `tsAddArg` 作为 post-parse 纯计划变换：关闭时保留 fragment query，开启且输入为空时清除 query，非空时替换 query；key 和 MAP URL 不参与改写。工具区 HLS 草稿是唯一 renderer owner，并从当前 manifest URL 的小写 `.m3u8?` 后提供默认输入。启用后静态下载必须走本地 plan；master 先选择具体 variant，直播把同一设置经受控 IPC 交给 recorder 的每轮 snapshot，参数值不写任务日志。
