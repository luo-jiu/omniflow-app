...
- 同一 level identity（`PATHWAY-ID / BANDWIDTH / RESOLUTION / FRAME-RATE / CODECS / VIDEO-RANGE / HDCP-LEVEL`）和同一解析后 URI 被重复声明时，pure parser 只保留一个 variant，并按首次出现顺序去重合并全部 AUDIO/SUBTITLES group。兼容单值字段继续指向首组，plan 同时携带完整 group 数组；工具区的音轨/字幕过滤、当前组展示和候选高亮都消费完整集合。显式 pathway 的跨 URI fallback 尚未进入可执行 DTO，当前不会为合并而静默丢弃备用 URL。
