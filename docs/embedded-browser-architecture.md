...
- AES-128 key 没有显式 `IV` 时，固定 hls.js 会为每个 fragment 使用原始 media sequence 的 16-byte big-endian IV。pure parser 会把该有效 IV 固化到 fragment DTO；本地 key 文件仍按 method/URL 去重，但 rewritten playlist 会按 method/local URI/IV/KEYFORMAT 状态变化重新输出 `EXT-X-KEY`，不能因 key URL 相同而吞掉 IV 轮换。真实 ffmpeg/ffprobe 门禁同时覆盖显式 IV 和非零 media sequence 的隐式 IV。
