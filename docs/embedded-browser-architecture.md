...
- DASH 计划下载已由 main 侧 `DashTaskExecutor` 统一执行：分片和 SegmentBase SIDX index range 只能通过当前 tab 捕获的 opaque resource authority 取回；带有限当前窗口的 dynamic snapshot、单文件 SegmentBase、有限 timeline、SegmentList range 和合成 SIDX expansion 可进入同一输出 adapter。DRM、parser unsupported reason、空 dynamic window、非法 byte range 和未支持的多 Period 计划会在网络请求或保存对话框前拒绝；持续 dynamic refresh/live 录制以及真实 MPD/ffprobe parity 仍是迁移中的验证缺口。
- 工具区当前承接两条 HLS 主线：
