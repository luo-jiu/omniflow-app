...
- 固定版 hls.js 也会在 media playlist 缺少 `#EXTM3U` 或 `TARGETDURATION`、重复 `TARGETDURATION` / `PLAYLIST-TYPE`，以及在首个 fragment 后声明 `MEDIA-SEQUENCE` / `DISCONTINUITY-SEQUENCE` 时发出 `levelParsingError`，同样不会进入 `parseTs`。同步 parser 在计划创建前抛出相同 reason；同步 facade 没有复制 hls.js 的 evented loader 状态机。
