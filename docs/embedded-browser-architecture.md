...
- HLS parser 已按固定 hls.js 1.6.16 支持 `EXT-X-DEFINE` 的 `NAME/VALUE`、playlist URL `QUERYPARAM`、master 到 media 的显式 `IMPORT`，并在 URI line、quoted-string 与 hexadecimal attribute 中做单次替换；重复定义、缺失 query/import/reference 只保留并抛出第一条等价 parsing error。工具区显式解析 master child 时会传递 master variable list；live recorder 仍未从 main-owned master authority 恢复该列表，含 `IMPORT` 的直播 child 暂未闭环。
