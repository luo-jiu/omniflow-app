...
- full-segment `AES-128/AES-256/AES-256-CTR` key 没有显式 `IV` 时，固定 hls.js 会为每个 media fragment 使用原始 media sequence 的 16-byte big-endian IV，而 `EXT-X-MAP` init segment 使用 sequence zero；MAP 捕获其声明时的 key context，不能被随后 media segment 的 key 覆盖。pure parser 会把两类有效 IV 和独立 key 固化到 DTO。AES-128 继续在 rewritten playlist 中按 `MAP key -> EXT-X-MAP -> media key -> EXTINF` 的播放顺序交给 ffmpeg；ffmpeg HLS demuxer 不识别 AES-256 METHOD，因此 CBC/CTR 的 32-byte key、16-byte IV、加密 MAP 和 media bytes 在本地工作目录阶段按固定 hls.js Web Crypto 模式解密，只从 clear playlist 移除已消费的 AES-256 key state。真实 ffmpeg/ffprobe 门禁覆盖 AES-128 的显式/隐式 IV、AES-256 CBC/CTR AAC，以及 AES-128 加密 fMP4/H264 视频与独立 AES-128/AAC 音轨双轨合并。
