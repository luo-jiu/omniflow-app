...
- LL-HLS 的 `EXT-X-PART` / `PART-INF` 不进入 Cat Catch 等价下载分片列表：固定版 hls.js 把它们单独保存在 `LevelDetails.partList`，Cat Catch `parseTs` 只复制 `LevelDetails.fragments`。因此计划与直播累计只认已发布的完整 `EXTINF` 分片，兼容 DTO 中的 `partCount` 在该路径为 `0`，不会把 PART 与随后完整分片重复下载。
