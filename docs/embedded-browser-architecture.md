...
- full-segment `AES-128/AES-256/AES-256-CTR` key 没有显式 `IV` 时，固定 hls.js 会为每个 media fragment 使用原始 media sequence 的 16-byte big-endian IV，而 `EXT-X-MAP` init segment 使用 sequence zero；MAP 捕获其声明时的 key context，不能被随后 media segment 的 key 覆盖。pure parser 会把两类有效 IV 和独立 key 固化到 DTO；本地 key/map 文件仍按资源去重，但 rewritten playlist 会按 `MAP key -> EXT-X-MAP -> media key -> EXTINF` 的播放顺序以及 method/local URI/IV/KEYFORMAT 状态变化重新输出标签，不能因 key URL 或本地文件相同而吞掉 IV/key 轮换。真实 ffmpeg/ffprobe 门禁覆盖 AES-128 的显式 IV、非零 media sequence 隐式 IV，以及 AES-128 加密 fMP4/H264 视频与独立 AES-128/AAC 音轨的双轨合并；AES-256 与 AES-256-CTR 仍待补齐真实输出组合。
