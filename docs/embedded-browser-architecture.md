...
- DASH 计划下载已由 main 侧 `DashTaskExecutor` 统一执行：分片只能通过当前 tab 捕获的 opaque resource authority 取回，动态 MPD、DRM、parser unsupported reason、SegmentBase SIDX、非法 byte range 和未支持的多 Period 计划会在网络请求或保存对话框前拒绝；单文件 SegmentBase、有限 timeline 和 SegmentList range 可进入同一输出 adapter。真实 MPD/ffprobe parity 仍是迁移中的验证缺口。
- 工具区当前承接两条 HLS 主线：
