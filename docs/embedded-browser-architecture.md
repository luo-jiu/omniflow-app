...
- MSE 深度捕捉当前已补第一版增量写盘：`cat-catch-port/mse/runtime.ts` 在 `appendBuffer` 累积超过阈值后由 `capture/adapters/mse-page.ts` 通过 tokenized probe relay 把 chunk flush 给 main，`processing/mse-spool.ts#MseSpoolStore` 以 per-track queue、单轨/总量预算和 TTL 落到 temp spool file；后续 `save / merge / transcode` 以及 MSE 下载动作优先逐轨读取 file-backed 资源，而不是再次整段 base64 提取。自动下载会在 main 侧优先合并音视频，使用 tab-scoped `mse-download` task 和共享 AbortSignal，随后通过 `processing/mse-download-output.ts#stageMseDownloadResource` 写入现有 embedded-browser download staging root；写入、取消或完成事件交付失败时清理 owned file，成功后发布完成事件。`src/features/embedded-browser/workflows/captured-output-workflow-coordinator.ts#CapturedOutputWorkflowCoordinator` 在 renderer 侧持有应用级完成队列和单一下载订阅，library-detail listener 卸载/重挂载不会丢失待导入文件；失败/取消事件在无挂载页面时也会清理临时文件。页面导航、view/process 销毁、tab close 和 controller dispose 会清理 spool。浏览器初始化还会回收超过 24 小时的 download staging 遗留文件。条件式真实 fragmented-MP4 双轨 ffmpeg/ffprobe fixture 已验证 output merge，失败会清理 partial output，零退出也要求非空文件，flush 与最终 drain 也统一了初始化头裁剪；固定上游 parity、生产大媒体、真实页面、UploadManager handoff 和完整导入终态仍待验证。
