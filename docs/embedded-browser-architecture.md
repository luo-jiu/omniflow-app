...
- `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 是当前共享的 main-side ffmpeg process adapter：HLS manifest/track、DASH local-file merge、MSE merge、captured-resource transcode 和 media-tool operation 均通过它执行。它统一处理 `-progress` 输出、AbortSignal、跨平台 process-tree 终止、失败 partial output 清理及零退出后的非空文件校验，并登记活动任务；应用退出时 `main.ts` 调用其 `dispose()` 等待并终止活动进程。跨入口用户取消、staged output lease 和 ffmpeg 可执行探测等其他生命周期边界仍待接入。
