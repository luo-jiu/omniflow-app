...
- `processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 是当前共享的 main-side ffmpeg process adapter：HLS manifest/track、DASH local-file merge、MSE merge 和 captured-resource transcode 均通过它执行。它统一处理 `-progress` 输出、AbortSignal、跨平台 process-tree 终止、失败 partial output 清理及零退出后的非空文件校验；应用级 task registry、app-exit cleanup 和媒体工具等其他 ffmpeg 入口仍待接入。
