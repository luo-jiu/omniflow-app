...
- HLS 跨 main/preload/renderer 的 DTO 由 `embedded-browser/contracts/hls.ts` 唯一定义，纯 parser 与 plan projection 分别由 `cat-catch-port/hls/parser.ts` 和 `plan.ts` 持有。main、preload 与 renderer 生产调用方都直接 import target contract/port；旧 renderer model 在原子 cutover 前只保留同名 re-export 和 parity test cleanup sentinel，三类函数必须与 pure owner 保持同一引用。
- HLS 本地 key/map/segment 下载、playlist 重写和 local -> ffmpeg 阶段序列由 `embedded-browser/processing/hls-task.ts#HlsTaskExecutor` 持有，首次计划与失败分片 retry 共用同一 executor；直播 poll/cumulative plan 由同目录 `HlsLiveTask` 持有。controller 继续负责 authority、保存路径、session、IPC 响应和产品日志；它通过注入的 ffmpeg adapter 执行输出，不在 target core 中泄漏 Electron UI。旧顶层 downloader/recorder 文件只保留同引用 compatibility export，等待 hls-engine 原子 cutover 删除。
