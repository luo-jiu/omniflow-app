...
- HLS `BYTERANGE` 必须保留固定 hls.js 的两种不同前序关系：媒体分片省略 offset 时使用紧邻前一个 fragment 的 range end，即使 URI 已切换；`EXT-X-MAP` 不接收 previous fragment，每个省略 offset 的 map range 都从 `0` 开始。不能把两者统一成按资源 URL 累计器。
