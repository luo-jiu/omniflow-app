...
- Main 捕捉设置持久化
  - `electron/service/embedded-browser/integrations/capture-settings-store.ts`
- View 生命周期
...
- DASH 计划下载已由 main 侧 `DashTaskExecutor` 统一执行：分片和 SegmentBase SIDX index range 只能通过当前 tab 捕获的 opaque resource authority 取回；带有限当前窗口的 dynamic snapshot、单文件 SegmentBase、有限 timeline、SegmentList range 和合成 SIDX expansion 可进入同一输出 adapter。`processing/dash-live-task.ts` 提供 main-owned 的注入式 dynamic snapshot refresh/dedupe 生命周期，`processing/dash-live-adapter.ts` 在 main 侧以 8 MiB 上限读取 MPD、拒绝 DTD/ENTITY 并转换为纯 AST/计划；`processing/dash-task.ts#appendDashRepresentationSegments` 只追加新 Representation 分片，`dash-live-session-owner.ts` 持有 session、task snapshot 和 tab/view/退出清理。DASH live start/stop/discard 已通过独立 IPC/preload bridge 暴露，但当前没有 renderer UI，且持续输出仍需真实 MPD/ffprobe 证据。DASH live 的 MPD URL 必须与被兑换的 captured resource URL 完全一致，否则在首次请求前拒绝，不允许退回无 authority 的 browser-session fetch。DRM、parser unsupported reason、空 dynamic window、非法 byte range 和未支持的多 Period 计划会在网络请求或保存对话框前拒绝。
...
- DASH live 停止后的冻结轨道合并也经过同一 `publishStagedOutput` lease，只有 ffmpeg 成功并完成 claim 后才结束 live session、删除 recorder workdir；轮询期间的 active recorder workdir 由 live session owner 单独管理并按 tab/view/退出生命周期清理。固定 Cat Catch 不持久化 active recorder，因此跨页面/跨进程续录不是当前 owner 的 parity 职责。
- 已捕获资源转码也通过 `publishStagedOutput` 先写 lease，再发布到用户目标；输出格式、保存对话框和 renderer 最终 `outputPath` 契约保持不变。转码执行现在由 `runRegisteredEmbeddedBrowserTransfer` 按 tab 登记，关闭 tab、view 销毁或 renderer 崩溃会通过共享 `AbortSignal` 终止 ffmpeg 并清理 partial output；它仍不自动进入 UploadManager handoff。
- MSE 手动音视频合并也通过 `publishStagedOutput` 保护最终用户目标，并与转码一样登记到 tab-scoped processing task；MSE 自动完成/周期保存仍写入 embedded-browser download staging root 并发布 download/import 事件，两者交付语义保持分离。
- 工具区当前承接两条 HLS 主线：
...
- master variant 列表按固定 hls.js 1.6.16 的 `parseMasterPlaylist -> MANIFEST_PARSED data.levels` 生成：`EXT-X-I-FRAME-STREAM-INF` 不进入可选普通 level；只要存在至少一个全部 codec 可识别的普通 level，就剔除混入未知 codec 的 level；没有普通 level 的 master 直接拒绝。若所有普通 level 都只含未知 codec，pure parser 会保留它们交给 ffmpeg 边界尝试，而不把 renderer 的 MediaSource 支持结果当作 main 执行权限。
- 同一 level identity（`PATHWAY-ID / BANDWIDTH / RESOLUTION / FRAME-RATE / CODECS / VIDEO-RANGE / HDCP-LEVEL`）和同一解析后 URI 被重复声明时，pure parser 只保留一个 variant；即使重复 URI 被其他 URI 隔开，也会按首次出现顺序去重合并全部 AUDIO/SUBTITLES group。兼容单值字段继续指向首组，plan 同时携带完整 group 数组；工具区的音轨/字幕过滤、当前组展示和候选高亮都消费完整集合。固定 hls.js 的 `MANIFEST_LOADED` 会保留跨 URI 声明并为无显式 pathway 的项生成 synthetic pathway，但 Cat Catch 实际生成选择项所消费的 `MANIFEST_PARSED` 只暴露首 URI；显式相同 `PATHWAY-ID` 时还会把后续 URI 的 group 挂到首 URI。OmniFlow 为避免数据损失，保留每个 identity/URI 为独立可选项，不赋予它们 fallback 顺序或 failover 语义。
- master rendition 列表对应 Cat Catch `MANIFEST_PARSED` 的 `audioTracks + subtitleTracks`，只接受大小写精确的 `TYPE=AUDIO` / `TYPE=SUBTITLES`。CLOSED-CAPTIONS、未知、缺失或小写 TYPE 不进入工具区选择，也不能借 captured master 获得 child authority；合法无 URI 音轨保留空 URL，缺失 NAME 回退 LANGUAGE，缺失 DEFAULT/AUTOSELECT/FORCED 归一为 `false`，与固定 `parseMasterPlaylistMedia` 输出一致。
- master `EXT-X-SESSION-KEY` 不作为下载 key 传播给 variant/rendition 或 child fragment。固定 Cat Catch 只用默认 `emeEnabled=false` 创建 hls.js，且下载页的 `MANIFEST_PARSED` / `LEVEL_LOADED -> parseTs` 不读取 `sessionKeys`；因此 pure parser 只执行该标签 quoted/hex attribute 的有序变量替换，以保留上游 master parsing error，随后明确丢弃 session key。child 只有自身声明 `EXT-X-KEY` 才进入 fragment key state。
- `EXT-X-KEY` 按 Cat Catch 固定的 full EME hls.js build 处理：full-segment `AES-128/AES-256/AES-256-CTR`、identity `SAMPLE-AES` 及 FairPlay/Widevine/PlayReady/ClearKey 的样本级方法进入 key state；不支持或大小写无效的方法不覆盖前值，`METHOD=NONE` 清空全部格式。不同 KEYFORMAT 作为同组备选并继承，identity 优先、单个非 identity 可直接形成 decryptdata，多个非 identity 未选择 key system 时只保留 encrypted 状态；显式 IV 按固定 `AttrList.hexadecimalInteger` 转为字节，保留其奇数位左补零和无效 `parseInt` 写入 `Uint8Array` 后归零的经验行为，再序列化为小写十六进制 DTO；缺省或空 IV 的 full-segment key 按 media sequence（MAP 为 0）派生并把 effective KEYFORMAT 归一为 identity。
- HLS manifest 解析的 main-owned inspection、静态双轨本地计划的 master/child playlist，以及 live recorder 的首次 manifest 请求，在普通 HTTP 响应失败时会按 Cat Catch 经验分支仅再请求一次同 URL，并设置 `cache: force-cache`；该回退继续使用 opaque resource authority 恢复 main-owned headers，不接受 renderer 自定义凭据。无响应的网络异常、MPD、分片和后续直播轮询都不会触发这一分支。
- LL-HLS 的 `EXT-X-PART` / `PART-INF` 不独立进入 Cat Catch 等价下载分片列表：固定版 hls.js 把 PART 保存在 `LevelDetails.partList`，Cat Catch `parseTs` 只复制 `LevelDetails.fragments`，所以兼容 DTO 的 `partCount` 仍为 `0`。但 parser 会先把每个 PART duration 累加到当前 fragment：常规顺序下随后完整 `EXTINF` 会覆盖该值；若 URI 在 EXTINF 前出现，累加值成为实际 fragment duration；若 PART duration 非有限，紧随 URI 不会物化，直到下一条有效 EXTINF 恢复。pure parser 必须保留这层 duration/URL/sequence 影响，同时不能把 PART URL 当下载分片。
- 正数 `EXT-X-SKIP:SKIPPED-SEGMENTS` 不进入下载计划。固定版 hls.js 会先把 skipped 项表示成 `null` fragment，再向外触发 `LEVEL_LOADED`；Cat Catch 的外部 listener 在内部 delta merge 之前执行，`parseTs` 直接读取 `fragment.url`，因此该路径本身不能形成有效下载列表。同步 facade 抛稳定错误，避免当前手写 parser 忽略 SKIP 后从错误 media sequence 下载；非法或 `0` skip 仍保留显式分片，重复正数 SKIP 的结构错误优先。若未来支持 delta，必须以有前序 snapshot 的完整 merge fixture 单独迁移，不能把 sequence 偏移当成 merge。
- HLS `BYTERANGE` 必须保留固定 hls.js 的不同前序关系：媒体分片省略 offset 时使用紧邻前一个 fragment 的 range end，即使 URI 已切换；MAP 自身 BYTERANGE 省略 offset 时从 `0` 开始，不接收 previous fragment。另有一个下载相关例外：尚无正时长 `EXTINF` 且 MAP 自身没有非空 BYTERANGE 时，紧邻并位于 `EXT-X-MAP` 前的独立 BYTERANGE 会同时绑定 init segment 和下一 media fragment；该独立 range 省略 offset 时同样继承 previous fragment end。正时长 `EXTINF` 后出现的 MAP 不接收该 media range。显式 length/offset 按固定 `parseInt` 接受合法整数前缀；非正 length、NaN 或负 offset 会在计划创建前稳定拒绝，不能退化成无 `Range` 的整资源下载。不能把这些关系统一成按资源 URL 累计器。
- 缺失、空或纯空白 `URI` 的 `EXT-X-MAP` 不得进入下载计划，也不得继续沿用上一张 MAP。固定 hls.js 对前两者创建 `url=""` 的新 init segment，纯空白 URI 则解析成 playlist URL；Cat Catch 随后会把扩展页面或 manifest 内容当初始化字节。同步 parser 把这些不可执行终态收敛为稳定错误，main downloader 不接触对应错误 URL。
- HLS segment URI 经 `EXT-X-DEFINE` 变量替换后为空时不得被静默丢弃。固定 hls.js 会保留位于中间的 `url=""` fragment，并让它占用 duration、sequence 和 key/MAP context；Cat Catch 随后把空 URL 交给下载器。pure parser 接受稳定拒绝差异并在计划形成前报错，避免遗漏分片后把所有后续 sequence 与隐式 IV 前移。
- HLS URL 在 plan 中保存最终可请求的 WHATWG canonical URL，而不是机械保留固定 hls.js `url-toolkit` 的中间字符串。Cat Catch 的 raw URL 可包含空格、编码 `..`、反斜杠或越过 origin root 的 parent traversal，但后续浏览器 `fetch` 会再次 canonicalize；OmniFlow 提前得到同一有效网络目标，使 main 能以精确 URL 命中当前 tab 已捕获资源并恢复 Cookie/Authorization。raw 字符串不同是明确的平台差异，网络目标和 authority 边界必须有 fixture/integration 保护。
- full-segment `AES-128/AES-256/AES-256-CTR` key 没有显式 `IV` 时，固定 hls.js 会为每个 media fragment 使用原始 media sequence 的 16-byte big-endian IV，而 `EXT-X-MAP` init segment 使用 sequence zero；MAP 捕获其声明时的 key context，不能被随后 media segment 的 key 覆盖。pure parser 会把两类有效 IV 和独立 key 固化到 DTO。AES-128 media 与普通加密 MAP 继续在 rewritten playlist 中按 `MAP key -> EXT-X-MAP -> media key -> EXTINF` 的播放顺序交给 ffmpeg。CBC 加密 MAP 带 BYTERANGE 时是窄例外：固定 hls.js 按未加密 length 向 16-byte block 补齐请求，非零 offset 还向前取一个 ciphertext block 并用它重置 IV；本地 task 对 AES-128/AES-256 执行同一 range/IV 语义、写出精确明文长度的 clear MAP，并移除已消费的 MAP key state。`1..15` 的非零 offset 无法提供完整前块，稳定拒绝而不夹成 offset zero。ffmpeg HLS demuxer 不识别 AES-256 METHOD，因此其余 AES-256 CBC/CTR 的 32-byte key、16-byte IV、加密 MAP 和 media bytes 也在本地工作目录阶段按固定 hls.js Web Crypto 模式解密。真实 ffmpeg/ffprobe 门禁覆盖 AES-128 的显式/隐式 IV、AES-256 CBC/CTR AAC，以及 AES-128 加密 fMP4/H264 视频与独立 AES-128/AAC 音轨双轨合并。
...
- HLS 清单切行必须与固定 fast parser 一致接受 `CR`、`LF` 和 `CRLF`。不能用只覆盖 `\r?\n` 的预处理提前拒绝纯 CR 清单；三种行边界进入相同 parser 和 plan owner。
- HLS 行首也不能统一 `trimStart()`：固定 fast regex 在 `#` 前紧邻 ASCII 空格时先命中 URI alternative，把整行标签文本作为零时长 fragment；只有 tab 等不会被该 alternative 消费的缩进才继续命中 tag。这个顺序会改变 media sequence、URL 和 AES implicit IV，且空格前缀的 `EXTM3U` 必须继续拒绝。
- master playlist 中一个 `EXT-X-STREAM-INF` 到其 URI 之间的 `#` 行属于固定 pending regex：后续 `STREAM-INF` 或 `DEFINE` 不能反向覆盖首 variant 或创建变量；`EXT-X-MEDIA` 仍由上游在首轮变量收集完成后的独立 media scan 投影，因此允许引用后置 `DEFINE`。逐行 port 必须显式保留这两个不同扫描边界。
- master AUDIO/SUBTITLES rendition 的 `DEFAULT/AUTOSELECT/FORCED` 直接复刻固定 `AttrList.bool`：仅精确大写 `YES` 为 true，`yes/YeS` 等非规范值保持 false，不能用不区分大小写的通用布尔 helper 改变音轨选择语义。
- 固定 `AttrList.parseAttrList` 只 trim attribute 名称，不 trim 未加引号的 value；pure parser 必须把原始 tag payload 交给同一正则边界。`METHOD=AES-128 ` / `METHOD=NONE ` 因此保持不受支持且不能错误替换或清除当前 key，`DEFAULT=YES ` / `AUTOSELECT= YES` 保持 false，带尾空格的 rendition `TYPE` 也不能被悄悄提升为有效 AUDIO/SUBTITLES。quoted value 的内部空格继续属于值本身。
- master variant 的 typed quality 字段直接复刻固定 `AttrList.decimalInteger/optionalFloat`：BANDWIDTH 与 AVERAGE-BANDWIDTH 用 radix 10 `parseInt` 并对超大正整数归一为 Infinity，FRAME-RATE 用 `parseFloat`；原始 RESOLUTION 与全部 `rawAttributes` 不做这层 typed 改写，工具区排序/标签和上游原始属性因此各有明确来源。
- Cat Catch `tsAddArg` 作为 post-parse 纯计划变换：关闭时保留 fragment query，开启且输入为空时清除 query，非空时替换 query；key 和 MAP URL 不参与改写。工具区 HLS 草稿是唯一 renderer owner，并从当前 manifest URL 的小写 `.m3u8?` 后提供默认输入。启用后静态下载必须走本地 plan；普通 master 先选择具体 variant，独立音轨组合由 main 生成 video/audio 双 plan 和隔离 workdir，两轨共享任务取消且任一失败会先取消并等待另一轨；直播把同一设置经受控 IPC 交给 recorder 的每轮 snapshot。参数值不写任务日志或安全投影。
- HLS 跨 main/preload/renderer 的 DTO 由 `embedded-browser/contracts/hls.ts` 唯一定义，纯 parser 与 plan projection 分别由 `cat-catch-port/hls/parser.ts` 和 `plan.ts` 持有。main、preload、renderer 与测试都直接 import target contract/port；旧 renderer model 已随 `hls-engine` 原子 cutover 删除。
- HLS 本地 key/map/segment 下载、playlist 重写和 local -> ffmpeg 阶段序列由 `embedded-browser/processing/hls-task.ts#HlsTaskExecutor` 持有，首次计划与失败分片 retry 共用同一 executor；直播 poll/cumulative plan 由同目录 `HlsLiveTask` 持有。controller 继续负责 authority、保存路径、session、IPC 响应和产品日志；它通过注入的 ffmpeg adapter 执行输出，不在 target core 中泄漏 Electron UI。旧顶层 downloader/recorder compatibility export 已随 `hls-engine` cutover 删除。
- HLS 分片下载器会在每个失败 attempt 上从同一 fragment 重新生成请求，因此 `BYTERANGE` 的 `Range` 在重试中保持不变。Cat Catch 使用递增延迟重试，OmniFlow 当前使用有界立即重排队；取消会原子切换 aborted 终态、清空待重试队列并中止正在执行的 retry，取消后不得进入 processor、completed 或有序写出。
- HLS/DASH 平台写盘通过 `TransferEngine` 的异步 buffer sink 进入同一个 worker 终态：sink 未完成时该 worker 不领取下一分片，成功字节不再留在 ordered buffer；sink failure 单独上报并进入 failed 终态，不消耗自动网络 retry。HLS 按独立 segment 直接写盘并清理失败 partial；DASH 按独立 part 写盘后以固定 1 MiB 缓冲顺序拼接，live append 在拼接失败或取消时回滚原文件长度。这样 pending write 数最多等于 thread，首片慢或乱序完成不会再让 Promise/closure 持有全部后续分片。
...
- `processing/task-registry.ts#ProcessingTaskRegistry` 是 main-side 活动任务登记与退出等待 owner；`processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 通过它登记每个 ffmpeg 进程，外部命令、native `will-download` session、普通直接/捕获资源流式下载、手动 local-save/MSE resource export、自动/周期/工具触发的 MSE output，以及 HLS/DASH session owner 的 active task 都在真实终态前保持登记。HLS/DASH 分片统一由 `cat-catch-port/processing/transfer-engine.ts#TransferEngine` 接受单一外部 `AbortSignal`、执行并发/Range/retry/队列取消和有序输出；旧 `embeddedBrowserFragmentDownloader` 已删除。HLS manifest/track、HLS 计划下载、DASH local-file merge、MSE merge、captured-resource transcode 和 media-tool operation 均通过共享 executor 执行，统一处理 `-progress` 输出、AbortSignal、跨平台 process-tree 终止、失败 partial output 清理及零退出后的非空文件校验；普通直接/捕获、手动 local-save/MSE resource export、HLS/MPD 直拉、HLS 计划下载和 DASH 静态计划由 `StreamingTransfer`、`publishEmbeddedBrowserExtractedResource`、共享 manifest runner、`HlsTaskExecutor` 或 `DashTaskExecutor` 写入 owner-scoped `StagedOutputLeaseStore`，通过 `publishStagedOutput` 在处理完成后一次性发布到用户目标，MSE 自动 output 由 `stageMseDownloadResource` 写入 download staging 并在事件交付失败时清理，native 下载通过 `NativeDownloadSession` 负责 staging、进度、取消和终态清理。完成事件同时写入 main-side `DownloadHandoffStore`，renderer 通过只读 replay IPC 在 listener 重挂载或 app 重启后重新取得仍存在的 staging 文件，成功交付/主动丢弃后确认移除 journal 条目；浏览器初始化时会按 24 小时 TTL 清理 download staging root 中上次运行遗留的文件。controller `dispose()` 会先取消并等待 registry 中的 tab-scoped output，再释放 capture/MSE/session owner；应用退出时 `main.ts` 随后调用 registry `dispose()` 收口剩余全局任务。隔离真实 Electron smoke 已证明 native `DownloadItem`、普通 streaming output、file-backed MSE output、MSE 双轨 merge、HLS/DASH 静态协议传输和一条 production FFmpeg staged output 的按 tab 用户取消：每个 filter 只命中目标 task；FFmpeg signal 传播到真实进程并等待 process tree，MSE merge 在两个 HTTP 轨道都开始传输后中断，HLS/DASH 在传输中止后跳过 merge，最终 target、partial、lease、workdir/part 与 registry 均清理。调用点审计还确认 manifest 单/双轨、HLS 本地轨、DASH 单/双轨、HLS/DASH live stop/export、MSE merge/transcode 都从 tab-scoped owner 取得并透传同一 signal；wrapper 定向 contract 与共享 executor 真实宿主证据共同闭合 FFmpeg 生命周期，不再按每个薄 wrapper 复制 smoke。media-tool 没有用户取消入口，但 executor 的 `kind=ffmpeg` 登记仍保证应用退出收口，它不属于 Cat Catch parity。UploadManager handoff/delivery terminal 的真实资料库确认和完整应用级 crash recovery 仍是 output-integration 的开放边界；系统保存对话框中的“取消”是独立交互验收，不由 task registry 证据替代。active HLS/DASH 录制跨进程续跑不是固定上游能力，未来若产品需要只能作为独立增强设计。

宿主取消证据补充：`ProcessingTaskRegistry.run` 现在持有普通 AbortSignal output 的共享 registration/settled/release 生命周期，controller 不再内置重复 helper。同一隔离 Electron smoke 通过慢速 HTTP、`StreamingTransfer` 与 `publishStagedOutput` 在取消前实际写出 65,536 bytes，随后按 tab 只取消一个 streaming task，并确认目标文件、流式 partial、磁盘/内存 lease 和 registry 全部清理；file-backed MSE 在 256 MiB source 的 partial copy 阶段中断且不发 completion；HLS/DASH 又分别通过 production session owner 和 task executor 在慢速分片传输 65,536 bytes 后取消，确认 AbortError、merge 未调用以及 target、lease、workdir、DASH part 和 registry 清理；真实 FFmpeg 在写出 32 bytes partial 后取消 outer task，确认 process tree 收口；production MSE 双轨 merge 则用两个真实 fragmented MP4 慢速输入，在视频 `30,720` bytes、音频 `512` bytes 后取消并清理 staged output；native `DownloadItem` 取消证据也保持通过。FFmpeg 不可发现时相关 smoke 明确报告 `supported=false`。因此上段开放边界现在特指系统保存框、资料库、完整登录态应用与 Windows，不包含已经覆盖的 native、普通 streaming、file-backed MSE、MSE merge、HLS/DASH 静态传输、共享 FFmpeg staged output 或已审计的薄 wrapper。

- live HLS 走显式“开始录制 / 停止录制”主线，停止后才交给 `ffmpeg` 导出。切换到另一条 HLS 请求或卸载工具组件时，会把未导出的 live session 当作放弃处理，并清理该任务冻结的输出目录；这与固定上游的页面局部 `recorder` / `recorderLast` / `fileStream` 生命周期一致，上游 `beforeunload` 也会 abort 文件流，不能把 active session 描述成可跨卸载或跨进程恢复。recorder 的 manifest/segment 请求共用一个 `AbortController`，discard 会中止在途请求；`EmbeddedBrowserHlsSessionOwner` 按 tab/request 归属 active direct/track/plan/retry/live-export 任务、retry/live 会话和最多 32 条最新安全任务投影。生产 controller 把页面导航、tab close、view 销毁、render-process loss 和 controller dispose 都收敛到唯一 HLS host lifecycle：先取消并等待匹配的 active fetch/ffmpeg，再清理任务投影、discard recorder、删除 workdir；close/close-all IPC 和应用 graceful shutdown 都会等待该清理完成，已经被先前生命周期事件取走的 session cleanup 也会纳入 dispose 等待。manifest/track ffmpeg runner 在取消或非零退出后删除 partial output。
- 工具区也可直接发起 HLS key 验证；候选会合并 manifest key URL、当前 tab 已捕获 key 资源和工具区手动输入 key。
- staged output 发布到已有目标时先尝试同文件系统原子 `rename`；Windows 或其他不允许覆盖的场景使用同目录临时备份，替换失败会恢复旧目标，跨设备复制路径复用同一回滚逻辑，避免交付失败造成用户已有文件丢失。
...
- MSE 深度捕捉当前已补第一版增量写盘：`cat-catch-port/mse/runtime.ts` 在 `appendBuffer` 累积超过阈值后由 `capture/adapters/mse-page.ts` 通过 tokenized probe relay 把 chunk flush 给 main，`processing/mse-spool.ts#MseSpoolStore` 以 per-track queue、单轨/总量预算和 TTL 落到 temp spool file；后续 `save / merge / transcode` 以及 MSE 下载动作优先逐轨读取 file-backed 资源，而不是再次整段 base64 提取。自动下载会在 main 侧优先合并音视频，使用 tab-scoped `mse-download` task 和共享 AbortSignal，随后通过 `processing/mse-download-output.ts#stageMseDownloadResource` 写入现有 embedded-browser download staging root；写入、取消或完成事件交付失败时清理 owned file，成功后发布完成事件并登记到 `processing/download-handoff-store.ts#DownloadHandoffStore`。renderer 通过 `embedded-browser:download-handoff:list/acknowledge` replay/确认接口取得跨 listener/app restart 仍存在的完成文件，`src/features/embedded-browser/workflows/captured-output-workflow-coordinator.ts#CapturedOutputWorkflowCoordinator` 继续持有应用级 `pending/importing/saving` 交付状态和单一下载订阅；完成队列把非敏感完成元数据写入带 24 小时年龄上限的 versioned journal，renderer/app 重启后恢复有效输出并把不可恢复的 in-flight 状态复位为 `pending`；auth-session cleared 会删除 renderer journal、清理非活动排队暂存并确认旧 main handoff，避免旧会话完成事件再次进入队列；活动交付不在登出时强行删文件，避免破坏上传竞态；失败/取消交付回到 pending，只有成功终态才清理临时文件，同一下载的并发交付会被拒绝。浏览器下载导入通过共享 UploadManager delivery adapter 分类 `completed/failed/cancelled/unknown`，并在创建 batch 时冻结 `libraryId`、`parentId`、文件名和相对路径。页面导航、view/process 销毁、tab close 和 controller dispose 会清理 spool。浏览器初始化还会回收超过 24 小时的 download staging 遗留文件。条件式真实 fragmented-MP4 双轨 ffmpeg/ffprobe fixture 已验证 output merge，失败会清理 partial output，零退出也要求非空文件，flush 与最终 drain 也统一了初始化头裁剪；显式大媒体 smoke 已通过同一 production merge/transcode owner 验证大于 128 MiB 的合并与大于 192 MiB 的解码输入。固定上游 parity、外部网站真实播放器页面、library target 冻结的真实宿主验证、crash quarantine 和完整应用重启宿主 smoke 仍待验证。

当前恢复边界补充：`DownloadHandoffStore.ensureReady()` 会在 renderer 第一次 replay 前剔除过期、缺失或越界 journal 记录并持久化结果；`StagedOutputLeaseStore.ensureReady()` 由 session 初始化主动触发，正常退出再由 controller `dispose()` 释放进程内 lease。两类软件启动/退出恢复已经接线，隔离 Electron 中的 renderer crash/replacement smoke 也已通过；完整登录态应用与跨平台宿主仍需验证。Lease path 和 claim token 只在 main 内用于原子发布；renderer 继续只消费最终路径或 download handoff，不新增 lease IPC。

真实宿主证据补充：`npm run cat-catch:smoke-host` 在隔离 macOS Electron 30.5.1 进程中先让真实 embedded `WebContentsView` 页面发起带 partition session Cookie 与 Authorization 的 loopback 媒体请求，production capture runtime 捕捉真实 session `webRequest`，renderer 只投影 `hasCookie/hasAuthorization` 而不含 header 值，main 再通过 opaque resource ID 兑换两项凭据并重放相同 46 bytes；随后启动慢速 64 MiB native `DownloadItem`，通过 production task registry 按 tab 取消并验证单任务命中、`cancelled` 终态、partial staging 删除和登记释放，再完成正常 native download、main handoff 持久化、renderer 强制终止、replacement renderer replay、生产文件 IPC 保存和 staging/journal 终态清理。`npm run cat-catch:smoke-large-host` 以流式生成的 64 MiB 确定性正常响应重复同一链，服务端不构造完整 fixture Buffer，staging 与最终输出都以文件大小和流式 SHA-256 验证。`npm run cat-catch:smoke-save-dialog` 还在 macOS 真实系统保存框中先取消、再确认隔离临时目标，随后继续走 production `fs:save-staged-download-file`，验证同字节输出、staging 与 journal 清理。三条命令验证 production boundary 的真实 Electron 页面捕捉、Cookie/Authorization authority、native registry 用户取消、crash/replay、bounded 大文件交付和 macOS 系统保存框，但不加载外部网站真实登录流程或完整 OmniFlow 登录态窗口，也不操作资料库或执行更长墙钟。

真实页面 probe 证据补充：同一 `cat-catch:smoke-host` 和 64 MiB 档通过 production document-start/current-document 注入入口，在真实 Chromium `WebContentsView` 中锁定 Deep `fetch`、inline script、`TextDecoder` 三条经验分支，以及真实 `MediaSource -> H.264 SourceBuffer -> appendBuffer` 的 4 bytes MSE 捕捉；document token 路由后的 main 投影分别保持 media/manifest/MSE kind。这里关闭的是隔离真实浏览器引擎中的 probe 安装和基础页面行为证据；文档其余位置的“真实页面”缺口现在特指外部网站、真实播放器状态与完整登录流程，不能用 loopback fixture 冒充。

Cat Catch 下载 fallback 平台边界：固定上游 `js/polyfill.js` 在缺失 `chrome.downloads` 时只执行无法监听终态、另存为或失败的 anchor click；`background.js` 的 error fallback 只针对右键 image-save，并打开带 referer 的隐藏 `downloader.html`，依赖扩展 tab/session/auto-close。Electron 不复制这套隐藏窗口状态机：native `DownloadItem` 由 main-owned `NativeDownloadSession` 收口，受保护资源的重试由当前 tab opaque authority 与 embedded session 执行，native failure 保留明确终态而不自动双写。该项按 capability map 的 `platform-substitute` 关闭，不再列为 output-integration 开放边界。

协议资源预算证据补充：`npm run cat-catch:smoke-protocol-multi-gib` 复用 HLS/DASH production task，让两条链分别处理 `256 x 4 MiB`，总计 2 GiB；macOS 运行 `19/19` 通过，整个 Vitest 进程峰值 RSS 为 `225,099,776` bytes、无 swap，临时目录全部清理。该证据只关闭多 GiB 协议分片写盘预算，不替代真实媒体有效性、ffmpeg 解码/合并、完整宿主或长墙钟。

FFmpeg 资源预算证据补充：`npm run cat-catch:smoke-ffmpeg-large` 通过 `mergeEmbeddedBrowserResourceTracks` 合并大于 128 MiB 的 H.264/AAC，并通过 `transcodeEmbeddedBrowserResource` 把大于 192 MiB 的 PCM 解码、编码为 MP3；FFprobe 验证流、时长和输出。macOS 测试耗时 5.78 秒，整次命令峰值 RSS 为 `125,829,120` bytes、无 swap，临时目录全部清理。该证据关闭本地 production merge/transcode 的大媒体预算，不替代真实网站、完整登录态或跨平台宿主。

直播连续性证据补充：HLS/DASH live 各自通过 32 个重叠 manifest/snapshot 窗口验证累计去重、单调交付和停止后的 timer 清理；HLS 还验证了真实本地 downloader 生成的 34 个唯一分片、规范化 playlist 和首尾字节。`npm run cat-catch:smoke-live-wall-clock` 另以真实 timer 同时运行 production `HlsLiveTask` 与 `DashLiveTask + appendDashRepresentationSegments`；2026-08-30 的 5 分钟 macOS 正式档得到 HLS 201 个唯一分片、DASH 301 个唯一分片，stop 后无续轮询，输出和 part 清理正确，整进程峰值 RSS `138,690,560` bytes、无 swap。该结果关闭 bounded 5 分钟 production live 墙钟证据，不替代真实网站、完整应用、跨平台或更长夜间运行。应用进程重启后继续 active 录制不属于固定 Cat Catch parity；未来若引入 durable recorder，需要独立定义持久化、凭据、workdir、幂等和清理边界。

公开网络媒体证据补充：`npm run cat-catch:smoke-real-media` 同时覆盖公开 HLS 与 DASH。HLS 使用 Mux 测试流，production parser/plan 从 master 选择最低码率 `320x184` variant，下载前 3 个真实 TS、重写本地 playlist，再由 production FFmpeg 输出 `864,407` bytes、`30.000181s` 的 H.264/AAC MP4。DASH 使用 Akamai 静态 MPD，production XML adapter/parser 选择最低码率 `320x180` H.264 视频与 `64k` AAC 音频，下载每轨 init 和前 3 个真实媒体分片，再由 production task/merge 输出 `411,257` bytes、`12.032s` 的 H.264/AAC MP4。2026-08-30 的 macOS 运行中 11 个媒体 URL 均只请求一次，FFprobe 和临时目录清理通过，整进程峰值 RSS `139,296,768` bytes、无 swap。该结果关闭公开真实 HLS/DASH 从 parser 到 output validation 的网络媒体证据；真实 Electron 页面捕捉与 Cookie/Authorization opaque authority 已由 host smoke 覆盖，外部网站真实登录流程和完整应用交付仍需各自宿主证据。

这些 hook 应继续通过 `services/*.api.ts` 调 preload bridge，不要直接在 hook 或组件里散落原始 bridge 调用。

file-backed MSE 输出此前虽然在 tab-scoped `mse-download` task 内，但底层 `copyFile` 不能响应 AbortSignal，取消会等待整份 spool 文件复制完才清理。`embeddedBrowserResourceFileSaveService.ts#saveEmbeddedBrowserExtractedResourceFile` 现在以 signal-aware stream pipeline 复制 file-backed 资源，base64 写入也接收同一 signal；自动 MSE staging 与手动资源导出都透传当前 tab signal。256 MiB 定向测试以及默认/64 MiB 两档真实 Electron smoke 都在 partial copy 出现后取消，并验证 AbortError、单任务命中、不发 completion 和 partial staging 清理。该证据只关闭 file-backed writer；MSE 双轨 merge 与 HLS/DASH 静态协议传输已有上文独立宿主证据，其他 FFmpeg wrapper 由 signal/owner contract 与共享 executor 证据覆盖。
...
- 完成下载 handoff 的重放快照与一次性确认
- 资源捕捉规则的持久化读取，以及网络捕捉 / probe 捕捉前的过滤判定
- 深度捕捉启动、probe 安装和 reload
- MSE 读取、导出、保存、合并、manifest 下载
- HLS 本地计划下载：`plan -> main-owned/embedded-session fetch -> local workdir -> local playlist -> ffmpeg -> staged output lease -> final output`
...
- HLS 分片参数：renderer 只提交可选 `segmentQuery`；字段缺失表示保持原 URL，空字符串表示清 query。main 不把该值投影到任务事件，只在解析每轮 media snapshot 后改写 fragment plan。
