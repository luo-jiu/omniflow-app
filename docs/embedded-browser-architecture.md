...
- `processing/task-registry.ts#ProcessingTaskRegistry` 是 main-side 活动任务登记与退出等待 owner；`processing/ffmpeg-executor.ts#FfmpegTaskExecutor` 通过它登记每个 ffmpeg 进程，外部命令、native `will-download` session 和普通直接/捕获资源流式下载也在真实终态前保持登记。HLS/DASH 分片 downloader 现在接受单一外部 `AbortSignal`，由 downloader 自身取消所有并发请求；HLS manifest/track、HLS 计划下载、DASH local-file merge、MSE merge、captured-resource transcode 和 media-tool operation 均通过共享 executor 执行，统一处理 `-progress` 输出、AbortSignal、跨平台 process-tree 终止、失败 partial output 清理及零退出后的非空文件校验；普通直接/捕获、HLS/MPD 直拉和 HLS 计划下载由 `StreamingTransfer`、共享 manifest runner 或 `HlsTaskExecutor` 写入 owner-scoped `StagedOutputLeaseStore`，通过 `publishStagedOutput` 在处理完成后一次性发布到用户目标，native 下载通过 `NativeDownloadSession` 负责 staging、进度、取消和终态清理。应用退出时 `main.ts` 调用 registry `dispose()` 等待并终止活动进程；HLS/DASH 直播、DASH 计划、MSE 其他 output、Cat Catch native BrowserWindow 下载 fallback、分片任务 registry 归属、协议任务、跨入口用户取消、local-save/UploadManager handoff 和 crash quarantine 仍待接入。
