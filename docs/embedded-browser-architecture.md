...
- HLS 本地 key/map/segment 下载与 playlist 重写由 `embedded-browser/processing/hls-task.ts#HlsTaskExecutor` 持有，直播 poll/cumulative plan 由同目录 `HlsLiveTask` 持有。controller 继续负责 authority、保存路径、IPC 响应、任务事件与 ffmpeg 产品编排；旧顶层 downloader/recorder 文件只保留同引用 compatibility export，等待 hls-engine 原子 cutover 删除。
