...
状态：WIP / Phase 2 已编码目录树到已打开网页的单文件投递，Phase 3 已编码，待跨平台验证
...
1. 本地文件拖入目录树，上传到指定目录。
2. 本地文件拖到已加载网页时遵循 Chromium 与网页自身的标准 drop 语义；OmniFlow 外壳不接管，明确文件打开继续使用打开命令或现有 Viewer。
3. 目录树文件拖到 Finder / Explorer / 桌面，导出为本地文件。
4. 目录树单个普通文件可拖入已打开网页的上传区；浏览器外壳和空白主页不提供拖入打开。
...
OmniFlow 的内置浏览器使用独立的 `WebContentsView`。主 renderer 与 embedded browser view 都显式设置 `navigateOnDragDrop: false`。因此网页没有接收文件时，不依赖 Chromium 自动导航到本地文件，也不由浏览器外壳补做拖入打开。

产品语义暂定为：

- 拖到已打开网页：只交给网页，不隐式替换当前页面。
- 拖到浏览器外壳、空白页或标签栏：不接管、不导航、不创建预览窗口。
...
- 已打开网页使用真实文件拖放语义。
- 浏览器外壳、标签栏和空白主页不接收文件打开 drop。
- 保留“按后缀映射网站打开”作为显式命令，不作为拖放失败回退。
- 明确网页接收、未接收和无法判定三种结果。

当前进度：目录树单个普通文件到已打开网页已编码。目录树在原有 HTML5 手势中附加内部 claim，
页面脚本捕获真实 drop 坐标，main 等待签名链接并把文件暂存到保留原始 basename 的隔离目录，
随后通过 CDP `Input.dispatchDragEvent` 发送 `dragEnter / dragOver / drop`。该链路不新增拖拽手柄，
不影响树内移动和 `DownloadURL` 桌面导出；文件夹、多选、归档目录和特殊节点不支持。

CDP `dragOver` 后会读取落点页面是否调用 `preventDefault()` 接受标准文件拖放；未接受时发送
`dragCancel`，不继续派发 `drop`，因此不会把普通页面区域伪报为已接收。

页面监听运行在 isolated world，并以 view 随机 nonce、可信原生事件、活动 tab 和一次性已注册 claim
共同约束 main 权限入口。拖拽单文件与全局已交付文件总量上限均为 1GB；导航和关闭 tab 会中止未完成
下载并清理半文件，已交付文件最长保留 30 分钟，正常退出同步清理，崩溃残留在下次启动按 24 小时
阈值回收。该上限不改变原有映射网站打开文件能力。

成功反馈只表示文件已交给网页，不表示网页上传成功。浏览器外壳、标签栏和空白主页不接管文件 drop；
`navigateOnDragDrop: false` 继续阻止 Chromium 把未支持的 drop 导航成新页面或独立窗口。普通本地文件到网页、
iframe 上传区和 Windows 实机验证尚未完成。
...
3. 浏览器外壳、空白主页和标签栏不支持拖入打开；需要预览时使用明确打开命令或现有 Viewer。
