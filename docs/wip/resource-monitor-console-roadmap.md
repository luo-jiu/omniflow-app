...
更新时间：2026-05-12
状态：进行中（Phase 1-3 已落地，Phase 4 部分落地）
...
- 资源分布和资源探针提供独立只读接口，前端并行加载，谁先返回谁先展示。（已落地）
- 前端新增内存态探针可用性图谱：打开面板后立即探测，之后每 5 分钟自动探测，只保留最近 60 次，重启后清空。（已落地）
...
- 资料库详情页资源分布和采样请求支持 `libraryId` 范围，保证回收站占用与当前资料库回收站跳转一致；资源探针按 provider / 基础设施级别展示，不按资料库过滤。（已落地）
...
- 增加采样存储。（已落地：`resource_monitor_samples`）
- 支持用户显式记录样本。（已落地）
- 显式采样支持 `dryRun=true`。（已落地）
- 支持趋势图。
- 支持阈值配置。
- 支持未来 MySQL / 外部资源探针配置。
...
- 手工验证资源监测到存储设置、迁移任务、当前资料库回收站的跳转关系；资料库详情页分布统计和采样请求带 `libraryId`，provider / 基础设施探针不按资料库过滤，仓库页入口不能在缺少 `libraryId` 时直接打开回收站。
- 手工验证“记录样本”写入成功 Toast 和返回样本 ID。
