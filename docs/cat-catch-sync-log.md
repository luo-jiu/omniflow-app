...
- affected capability IDs: `output.ffmpeg-process-owner`；metadata 当前为 `7 units / 32 capabilities / 101 cleanup entries / 229 planned IDs / 211 active refs`，状态为 `15 verified / 6 porting / 1 ported-unverified / 10 pending`。
- runtime changes: 新增 `electron/service/embedded-browser/processing/task-registry.ts#ProcessingTaskRegistry`，统一 main-side 任务登记、按 kind/tab/request 取消、settled 等待和幂等 dispose；`FfmpegTaskExecutor` 将每个 ffmpeg 进程登记到该 registry，应用 `main.ts` 在 graceful shutdown 中 dispose registry。HLS manifest/track、DASH local-file merge、MSE merge、captured-resource transcode 与 media-tool operation 继续由共享 executor 执行。
- fixtures/tests: 复用既有 HLS cancel/cleanup、DASH local-file output、MSE merge/transcode failure/success-contract 与真实 fragmented-MP4/HLS/DASH output 证据；新增 media-tool 成功非空产物/失败 partial output 测试，以及 `task-registry.test.ts` 的匹配取消等待和 dispose 幂等测试，重点验证入口迁移不改变旧 IPC 返回合同。
- accepted differences: registry 只收口 main-side task lifecycle，不改变各协议 parser、authority、workdir 或 renderer 语义；HLS/DASH/MSE session owner 仍保留其领域快照与 workdir 清理，跨入口用户取消和 staged output lease 尚未接入。
- excluded changes and reasons: 未迁移 ffmpeg 可执行探测、普通下载/外部工具/协议 task 注册、staged output lease、资料库导入、renderer UI 或 `dist-electron/**`；这些边界需要独立生命周期与产品交付决策。
...
## 2026-08-29: same target (DASH live stop-export staged output wiring)

- observedHead / migrationTarget: `2cb981d7c2f4614732edccc167c4b5793d1cb138`；游标不移动，本切片把 DASH 直播停止后的冻结轨道合并接入 staged output lease。
- reviewedThrough / portedThrough: 均保持 `null`；`dash.parser-planner` 与 `dash.timeline-download-merge` 继续 `porting`，`output.staged-output-lease` 继续 `porting`，dash-engine/output-integration unit 仍开放。
- change groups: `main-authority-wiring`、`output-ownership`、`live-terminal` 与 `partial-output-cleanup`。
- affected capability IDs: `dash.timeline-download-merge`、`output.staged-output-lease`；metadata 仍为 `7 units / 32 capabilities / 102 cleanup entries / 229 planned IDs / 226 active refs`，状态为 `15 verified / 11 porting / 1 ported-unverified / 5 pending`。
- fixtures/tests: DASH output、session owner、lease/publisher 与真实 ffmpeg/ffprobe 现有集合继续覆盖轨道合并、取消、workdir 清理和非空产物；本切片新增生产 stop-export wiring，未虚增 capability test ref。
- accepted differences: renderer 继续收到原有最终 `outputPath`；DASH live recorder 仍在停止前持有自己的 workdir，staged path、lease id 和 claim token 只在 main publisher 内部存在。
- excluded changes and reasons: 未接入 DASH 持续输出恢复、MSE、local-save、UploadManager、crash quarantine、renderer UI、`dist-electron/**` 或 upstream 游标；这些入口仍需各自 delivery owner 和终态证据。
- unresolved gaps: DASH live active recorder workdir / continuous output、DASH 计划更广泛 registry 归属、DASH/MSE 其他 output、统一协议任务 registry、lease crash quarantine/预算、application delivery terminal、真实页面和长时间大媒体仍待补齐。
