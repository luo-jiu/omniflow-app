...
- 当前确定性 Analyzer 使用 `@vscode/tree-sitter-wasm` 的 runtime、Bash grammar 与 PowerShell grammar；静态已知子集必须生成稳定 `analysisIdentity / authorizationIdentity`，动态语法、未知命令或参数、link-producing copy、交互参数、Zsh `=command`、named-home 展开和 Parser / WASM 故障均 fail-closed。electron-builder 必须只打入许可证、包元数据和这三份 WASM；2026-08-31 的 unsigned macOS arm64 unpacked 包已确认 ASAR 精确文件清单，并由打包 Electron runtime 从 ASAR 实际加载两种 grammar、分别解析无错误 Bash / PowerShell fixture。普通 `npm run build` 或源码目录 Vitest 不能替代该检查
