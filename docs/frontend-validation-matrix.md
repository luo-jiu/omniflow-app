...
- `/login` 在亮色、暗色和跟随系统主题下保持冷色背景、玫瑰面板、主卡与品牌强调的对应层级；未保存过主题且账号未配置主题偏好时默认跟随系统，已有明确亮色 / 暗色选择继续保留。粉色面板右上角的太阳 / 月亮按钮直接切换目标主题，支持鼠标与键盘且不影响表单提交。React 首次渲染直接使用已保存主题和系统偏好，避免挂载后的明暗跳变；背景、卡片、中心形体和登录 / 注册表单平滑过渡，新增表单控件轻微错峰进入，`prefers-reduced-motion` 下取消旋转、缩放与错峰动画；登录 / 注册两态及 `920px`、`560px`、`320px` 视口均不发生遮挡或溢出，窄屏始终保留切换到另一认证状态的入口且隐藏入口不进入 Tab 顺序
