...
- 新建与开发期已有 Agent SQLite 都保持 schema 2：唯一 Schema Coordinator 完成 reconcile 并发布 barrier 后，Session / Memory / Quota / Workspace Store 才能打开业务连接；`media.extractAudio@1` 与不可执行的 `shell.run@1` prepared action 分别按 `kind / version` 严格校验，Shell command hash 必须对应精确 command bytes。`prepared_action_id / prepared_action_json / prepared_snapshot_hash / approval_input_hash` 保持 `text`，审批 hash 与冻结快照 hash 必须一致。直接 Store 写入禁用环境变量、SQL 单列篡改、command/hash 漂移或把 ASCII JSON / ID / hash 写成 BLOB 都会在 trigger 或 bootstrap main normalizer 边界被拒绝；旧媒体 action 原地回填且损坏行使 bootstrap 整体回滚。workspace / quota ledger 幂等补齐，quota ledger 原地增加 unknown occupancy 标志并重建受影响 trigger，不新增 schema 3 / 4

#### Shell 目标实现门禁（当前尚不可执行）

生产 Registry 尚未注册可执行 `shell.run`。当前只允许验证非执行准备基座：exact command bytes/hash、Provider path-free public identity、可选 Run snapshot identity、AI destination binding、受控环境，以及 macOS / Linux PreparationService 在 Quota live lease 下对 `input / work / output / tmp / home` 运行 25 秒有界 scanner v3 并冻结 content / metadata identity，且始终不 spawn、不运行用户命令；准备期间允许受控扫描、quota 记账和失败清理。目录枚举必须在条目上限生效前停止、拒绝不能无损映射为 UTF-8 的原始名称；扫描必须拒绝链接、硬链接、特殊文件、越界、深度 / 计费字节 / 时间超限和可检测的中途漂移。相同文件系统快照、逻辑根、累积 provenance、根 POSIX mode 和 scanner revision 的不同物理 workspace 得到相同语义 identity，cwd 之外任一根的增删改、`ctimeNs`、allocated blocks、provenance 或物理根 mode 变化也会改变 identity；quota 只按 `max(data fork 读取字节, st_blocks * 512)` 校正。generic xattr、ACL 和平台 file flags 尚未由可信 handle 完整枚举或计量，因此必须保持 `persistentRuleEligible: false`；补齐 handle-based 平台 metadata adapter 前不得注册可执行 Shell 或复用权限规则。Workspace Store 的恢复和公开 mutation 与持久化必须共用单一 gate，失败只回滚当前 record，累计 provenance 超过 256 条时内存、SQLite 和重启恢复均不得被污染；preparation 在门内二次复核 record identity / generation。Workspace 与媒体 Store 的托管根还要在初始化冻结 canonical path 与 `dev / ino`：把 live 根替换为 symlink 或同路径新目录后，prepare / residue sweep / cleanup 都必须拒绝且保留外部 sentinel；预先存在的 symlink 根必须拒绝，缺失 legacy 媒体根不得在 sweep 时重建，可信 current + legacy 双根仍能正常清理。当前 root guard 仍是 path-based 复验，不是 OS containment；可执行前必须由 `openat`、directory handle 或平台等价 native anchor 收口其 TOCTOU。cleanup 与扫描并发时先在仍持有 lease 时持久化 `deleting` 与保守已观测字节，最后一个进程内 lease 释放后再尝试物理回收；删除失败必须保留 `deleting` 状态和计费并由 sweep 或启动恢复重试，不能重新进入 active。第一阶段持久化失败时不得调用 adapter，当前进程拒绝新的 reservation、正向扩容和 lease，下一次 sweep 必须先补写同一 owner 与债务的 `deleting` 事实，再调用 adapter；正常退出也必须尝试刷入，失败则明确报错。扫描无法完成或快照已经漂移时必须以 unknown occupancy 隔离，除保守字节债务外还要持久化独立 unknown 标志；该标志不会因下次提高配额而失效，物理删除成功前始终拒绝新 admission。普通 TTL 候选也必须先持久化 `deleting` 再物理删除；`releaseLease` 不为纯内存 lease 重写 SQLite。Windows content identity 在 reparse point、ADS、UNC 与大小写敏感 NTFS 真机验证前必须 fail-closed，不能因 PowerShell Provider 已发现而通过。下面是注册前必须补齐的完成门禁，不属于现有能力：

- dispose 与尚未进入 mutation queue 的 create / prepare / cleanup 并发时，先关闭新准入并等待已接纳操作完整收口，再关闭 adapter 和 persistence；迟到 adapter 回调被拒绝。重复 dispose 返回同一 Promise，closing 后公开异步方法稳定拒绝、同步 `get()` 返回 `null`
