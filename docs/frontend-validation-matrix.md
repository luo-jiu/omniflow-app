...
- Audio Archive 展开页实时频谱各柱只上下跳动；数据必须直接来自 `globalAudioPlayer` 唯一 Web Audio `AnalyserNode` 的当前 FFT 频率桶，不得为频谱再次下载整首音频、向 renderer 传递大体积 base64、完整解码 PCM 或启动整曲预计算任务。每帧将 `45Hz～16kHz` 对数频带映射为占画布 `80%` 的浅色原波，覆盖 `0%～80%`；同一组高度逐柱倒序镜像后形成同色系深色波，覆盖 `20%～100%`。深色镜像层先绘制，浅色原波后绘制并位于前层；有彩色封面时两档颜色保持封面主色相和清晰的明暗层次，无封面、灰色封面或取色失败时回退主题默认蓝色。两层共用同一组目标高度和时间平滑结果，组合外轮廓中心对称。底部进度条复用同一配色，已播放区域从 `00:00` 深色渐变到当前位置浅色并使用圆角端帽，不显示圆点 thumb；透明原生 range 必须支持点击、拖拽和键盘 seek，控制栏不显示打开普通音频 viewer 的按钮。整条画布对两层统一应用边缘约 `40%`、中心 `100%`、另一侧回到 `40%` 的平滑透明度渐变。每个波按自身进度使用同一条首尾归零高度包络：原波在画布 `0%/80%` 归零，镜像波在 `20%/100%` 归零。暂停后低于一个物理像素即钳为零并彻底清空，响应式柱数减少后隐藏区间的旧高度也必须清零，收起或切走 tab 后停止逐帧刷新；分析链路不可用时仍保持重叠镜像动态兜底且不影响播放。快速切歌、关闭 tab 和切换资料库后不得残留旧播放请求或分析任务；两个音频 tab 或 ASMR 与普通音频交替发起请求时，较慢的旧请求不得覆盖最后一次操作
