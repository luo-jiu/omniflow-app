...
- 六类目录图标都必须提供闭合 / 展开两态，目录树展开后切到对应 `*-open` 图标；切换由 Semi Tree 的 `icon(props.expanded)` 动态完成，不把展开状态写回 `treeData`
...
该授权在当前目录侧栏生命周期内保留：折叠后可直接再次展开，不会因为重新锁定并清空 children 而打断折叠动画。离开并卸载当前侧栏，或修改该目录的内置类型 / 归档配置后，仍需再次显式打开。
...
- 如果直属子目录本身也是 `VIDEO` 归档目录，则作为“合集”卡片展示；双击合集卡片会直接打开合集内排序最前的可播放视频，并把该合集一代视频单元的第一页作为 `VideoViewer` 播放列表传入，播放页底部控制条可展开列表气泡切换集数并继续分页加载更多；只看当前父子关系，不在父层递归拉平孙级内容；合集封面沿用视频单元规则，支持 `coverNodeId` 和合集目录第一代图片文件
- 为兼容历史数据，直属视频媒体文件仍会作为卡片展示；归档页不会为了同名封面 / 字幕全量扫描归档根目录，历史同名伴随资源建议逐步迁移到视频文件夹结构或显式 `coverNodeId`
- 不把 `mp3` 等音频算成视频卡片
- 如显式写了 `coverNodeId` 就优先用它；否则用视频文件夹内封面或视频首帧预览兜底
- 从视频归档卡片打开普通视频 viewer 时，字幕文件会作为库内字幕候选传入；存在多个字幕时默认加载排序最靠前的字幕，用户仍可在视频操作台切换

当前 `AUDIO` 归档目录展示的是歌曲列表：

- 优先支持直属 `AUDIO + archiveMode=0` 歌曲文件夹：文件夹本身是歌曲行，内部第一个音频文件作为主媒体，第一个图片文件作为封面，字幕 / 歌词文件作为伴随资源候选
- 从目录树双击 `AUDIO + archiveMode=0` 文件夹时，不再走“打开目录内第一张图片”的通用兜底；它会直接复用 `UnifiedAudioViewer` 的单歌曲展开模式，读取直属音频、封面和歌词，但不会自动开始播放
- 兼容历史直属音频媒体文件：普通 `mp3/flac/m4a` 等音频文件不需要设置内置类型，也会作为歌曲行展示；归档页不会为了同名封面 / 歌词全量扫描归档根目录，历史同名伴随资源建议逐步迁移到歌曲文件夹结构
- 不把字幕、文本、视频或图片单独算进歌曲行
- 上级音频归档不展示下级 `AUDIO + archiveMode=1` 子归档为合集；下级归档可以存在，但只在用户直接打开该归档目录时生效
- 目录树中同级同名字幕默认折叠隐藏；通过右键临时显示时，字幕紧贴对应音频文件下方
- 归档页底部内置播放控制条，点击封面可在归档页内展开大封面 + 歌词视图；不再跳转到普通 `audio` viewer
...
资源监测里的分类统计把“内置类型”视为内容集合，而不是只看文件节点自身：

- 文件如果位于非 `DEF` 内置类型节点内部，就归属该内置类型。
- 内置类型嵌套时，统计按最外层非 `DEF` 内置类型归属，避免同一集合被内层类型重复拆分。
- 找不到非 `DEF` 内置类型祖先时归为普通资源；没有任何文件节点引用的对象归为未归类对象。
- 归档模式允许嵌套，当前资源监测不按 `archiveMode` 做递归分类统计。
...
- `COMIC + archiveMode = 1` 表示漫画归档 / 系列目录
- 该归档视图只展示直属一代子节点
- 直属 `COMIC + archiveMode = 0` 目录展示为普通漫画卡片，打开后进入 `comic`
- 直属 `COMIC + archiveMode = 1` 目录展示为合集卡片，打开后继续进入 `comic_archive`
- 更深层级不会被上级归档递归铺平；从中间层直接打开时，也只从当前层开始建立返回栈
...
4. 非归档内置类型目录首次需要通过“打开原始目录”进入原始结构；授权后折叠、再次展开都有动画且不重新要求授权，侧栏重新挂载或目录配置变化后恢复未授权状态。
5. 归档目录拖拽上传会被拦截。
