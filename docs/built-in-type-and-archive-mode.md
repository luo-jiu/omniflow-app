...
当前 `VIDEO` 归档目录展示的是直属视频媒体文件卡片：

- 只认视频媒体资源，不把 `mp3` 等音频算进来
- 子视频文件当前不要求再单独设置 `builtInType = VIDEO`
- 如显式写了 `coverNodeId` 就用它，否则先显示占位卡面
...
8. `VIDEO` 归档目录双击会进入独立的视频归档页。
9. `AUDIO` 归档目录双击会进入独立的音频归档页，目录树默认隐藏同名字幕文件，右键可临时显示或再次隐藏。
