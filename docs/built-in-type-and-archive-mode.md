...
- 如果直属子目录本身也是 `VIDEO` 归档目录，则作为“合集”卡片展示；双击合集卡片会进入该子归档继续展开，只看当前父子关系，不在父层递归拉平孙级内容；合集封面沿用视频单元规则，支持 `coverNodeId` 和合集目录第一代图片文件
...
10. `VIDEO` 归档内的直属子视频归档会显示“合集”标签，双击后进入子归档，不提前展示孙级内容；进入合集后应出现返回父归档按钮。
