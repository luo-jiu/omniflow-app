...
- 资源组成主图支持「冠军分离」：当第一名占比达到 `60%` 且可展示项不少于 3 个时，冠军项单独渲染通栏条，其余项以自身最大值作为坐标上限放大展示；条上标签与 tooltip 仍展示真实占比和容量，不满足条件时保持普通线性条形图
- 到存储设置、迁移任务、回收站的快捷跳转
- 用户显式触发的历史样本记录
...
- 资源组成主图冠军分离：至少有 3 个正值项且第一名占比达到 60% 时展示通栏冠军条，其余项放大展示且标签为真实占比；正值项不足或分布均匀时保持普通条形图。
