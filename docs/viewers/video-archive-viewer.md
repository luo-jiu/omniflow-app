...
- 归档目录内如果直属子目录本身也是 `VIDEO` 归档目录，则以“合集”卡片展示；双击合集会直接打开合集内排序最前的视频，并把合集一代视频单元分页拉全后传给普通 `VideoViewer` 做播放列表
- 用卡片墙方式展示直属视频资源
- 解析并补齐 `coverNodeId` 对应封面；没有显式封面时，优先兼容同名 / 文件夹内封面，再用视频文件自身链接做首帧预览
- 解析视频单元内或历史同名字幕，双击打开普通视频 viewer 时作为库内字幕候选传入；合集只同步加载首播集字幕，其他集切换时再按需加载
...
- 归档嵌套：直属 `VIDEO + archiveMode=1` 子目录作为 `cardKind=collection` 合集卡片展示；双击合集卡片不会进入新的归档层，而是分页请求该合集的一代视频单元直到 `hasMore=false`，直接打开排序最前的可播放视频，并把同一批视频单元作为 `VideoViewer` 播放列表；合集封面沿用普通视频单元封面规则，优先 `coverNodeId`，否则取合集目录第一代图片文件

也就是说现在的模型不是“递归抓整棵子树所有视频”，而是按归档目录自己的第一代视频单元结果来展示。

归档嵌套也只看亲子关系：父归档只显示直属子归档为合集，不把孙级内容提前摊平。合集播放列表当前只使用该合集的一代可播放视频单元；如果合集里继续出现子合集，后续再按播放列表分组能力扩展，不在当前父层递归摊平。
...
1. 如果 `cardKind=collection`，先分页请求该合集目录的一代 `VIDEO` 归档卡片直到没有更多，过滤出可播放视频单元，组装 `videoPlaylist`
...
4. 收集字幕候选：
   ...
   - 历史直属视频文件按同级同名字幕兼容匹配
5. 合集首播只同步补齐第一集字幕；播放列表里其他视频单元只保存字幕所在卡片节点摘要，切集时由普通 `VideoViewer` 按需读取
...
11. 播放列表气泡能切换同合集视频，切换后仍停留在当前视频 tab，并带上同一份 `videoPlaylist` 和归档返回链路。
