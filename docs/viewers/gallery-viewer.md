...
4. 网格和详情顶部不直接显示文件名；悬停卡片仍能看到 title。
5. 图片详情按钮可打开媒体详情弹框；有 EXIF 的图片能显示拍摄时间 / 定位等字段，无 EXIF 时有明确提示。
6. HEIC / HEIF 在本机有 `ffmpeg` 时能生成缩略图和详情大图；详情弹框能展示 `sips` 可读的时间、相机、尺寸等信息。
7. 视频详情可播放、暂停、seek，并且开始播放后 MediaHub 出现视频 entry。
8. 从视频切换到图片或回到网格后，当前图集视频被释放。
9. 关闭图集 tab 后，图集视频从 MediaHub 消失。
10. 切换到其他 tab 再回来，图集 tab 不丢失当前前端状态。
