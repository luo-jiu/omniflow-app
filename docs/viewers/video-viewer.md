...
1. 主视频区域
   ...
   - 字幕覆盖层
2. 底部控制条
   - 覆盖在视频画面底部，不单独占用播放器高度
   - 鼠标移动时显示，静止约 3.2 秒后渐隐；拖动进度、聚焦控件或展开轻量面板时保持显示
   - 播放 / 暂停
   - 快退 / 快进
   - 与音频播放器一致的无圆点渐变时间轴；点击直接跳转，拖动时只预览，松手后提交实际播放位置
   - 音量按钮与竖向音量面板
   - 倍速按钮与竖向倍率面板
   - 合集播放列表按钮与气泡列表
   - 工具台显隐
...
- 手动加载本地 `.srt` / `.vtt` / `.ass` / `.ssa` / `.lrc` / `.qrc.xml`
...
控制条属于 `.video-shell` 内的画面覆盖层，因此进入 fullscreen 后仍可操作。右侧工具台本身不属于 fullscreen 根节点，全屏状态点击工具台按钮时必须先退出全屏，再打开工具台，不能留下只改状态但没有可见反馈的按钮。默认显示只用于刚进入页面和用户刚移动鼠标后的短暂反馈；静止时必须连同鼠标指针一起收起。只要用户正在拖动时间轴、键盘聚焦控制条、打开播放列表 / 音量 / 倍速面板，就不得触发自动隐藏；单纯把鼠标停在控制条上仍按静止处理。

时间轴使用透明原生 `range` 承担点击、拖动和键盘交互，可视轨道单独绘制为无圆点的蓝色渐变和圆角端头。拖动期间只更新临时预览位置与时间文本，不连续修改 `<video>.currentTime`；指针松开、键盘释放或控件失焦时才提交一次实际 seek，随后立即刷新观看进度。鼠标提交或取消后释放时间轴焦点，避免拦住播放器全局快捷键；通过键盘进入时间轴时仍保留焦点，支持方向键精细调整。

快退 / 快进按钮表达的是固定秒数 seek，不是上一条 / 下一条或分享动作。必须使用成对的媒体快退 / 快进图标，并保留明确的 `title` / `aria-label`。
...
6. 底部控制条在鼠标移动时出现、静止时渐隐；时间轴无圆点且使用蓝色渐变，点击可直接跳转，拖动时只更新预览时间、松手后才实际 seek；拖动进度、聚焦控件或打开面板时不消失，快退 / 快进图标语义正确，音量、倍速、工具台、宽屏、全屏仍可用。
7. 右侧工具台可以展开和折叠；宽屏模式会临时折叠目录树和右侧工具台，退出后恢复进入前状态。
8. 加载 `.srt` / `.vtt` / `.ass` / `.ssa` / `.lrc` / `.qrc.xml` 后，字幕会固定显示在主视频区域底部。
9. 清除字幕后，字幕覆盖层消失。
10. 没有字幕时不会影响普通视频播放。
