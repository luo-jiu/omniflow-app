...
当前映射包含 7 个 cutover unit、32 项能力、206 个上游 anchor、106 个 cleanup entry 和 156 个唯一计划测试 ID。9 项能力达到 `ported-unverified`，5 项为 `porting`，其余 18 项仍为 `pending`；111 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。
...
下一步把 `downloadEmbeddedBrowserHlsPlanResource` 的执行编排迁入 target task owner，再继续 HLS 完整 parser 标签语义；DASH 直拉与旧 catch toolkit 的过渡 DTO 分别留在 DASH/network unit 收口，非 HLS ffmpeg 的应用级 owner 留在 output-integration unit。证据完整前不进行 hls-engine cutover，也不提前删除兼容出口。
