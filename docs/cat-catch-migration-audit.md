...
当前映射包含 7 个 cutover unit、32 项能力、210 个上游 anchor、100 个 cleanup entry 和 205 个唯一计划测试 ID。15 项能力达到 `verified`，5 项为 `porting`，1 项为 `ported-unverified`，其余 11 项仍为 `pending`；184 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成固定目标下的原子 cutover；DASH 的 renderer parser facade、main task、live task、main XML adapter 和 output adapter 已接入目标代码（live task/adapter 仍未接生产持续刷新 IPC），但 `dash.parser-planner` 与 `dash.timeline-download-merge` 仍保持 `porting`，其余 3 个 unit 仍开放。
...
| DASH engine | target parser/task + main output adapter, production dispatch switched | `cat-catch-port/dash/parser.ts` 已覆盖 Period/AdaptationSet/Representation SegmentTemplate、SegmentList、SegmentBase 继承、SegmentBase SIDX metadata、BaseURL、模板 token、SegmentTemplate `endNumber`、SegmentList timeline、静态最后一片 duration 修正、有限及 availability-bounded `r=-1`、dynamic duration-only SegmentTemplate 的 client clock offset、同身份静态多 Period 串接、SegmentList range validation 和 DRM 投影；`processing/dash-task.ts` 已收口 SIDX index-range fetch、range、顺序写入、取消和失败清理，并允许带有限当前窗口的 dynamic snapshot 走同一下载/合并路径；新增 `processing/dash-live-task.ts`，由 main-owned task 管理注入式 MPD snapshot 的刷新节奏、按 representation 去重、增量分片回调、停止和取消；新增 `processing/dash-live-adapter.ts`，在 main 侧受限读取 MPD、拒绝 DTD/ENTITY、将真实 XML 转成纯 AST 并映射为 `DashTaskPlan`，测试覆盖动态 MPD、headers/signal、非 2xx、解析错误和大小限制；MPD 计划下载已通过 main-owned authority 接入新 task 和共享可取消 ffmpeg runner。持续 live IPC/output 交付、无 availability 证据的动态窗口、复杂嵌套 SIDX、不完整或初始化冲突的多 Period 集合、真实媒体输出与完整 unit 关闭仍未完成 |
...
6. 目前有 184 个唯一 active test ref；逐项名称与来源以 capability map 为准。Network/Deep/HLS 证据已支撑对应 unit cutover；MSE 的纯 runtime、spool 生命周期、relay 合同和 synthetic output contract 已有证据，但 production 大媒体与真实下载导入仍未完成；DASH parser-planner 与 timeline-download-merge 已接入生产目标但继续 `porting`，transfer/output 仍按各自开放状态判断。
