...
| 能力族 | 当前实现 | 已知结论 |
| --- | --- | --- |
| network capture | legacy production owner + unregistered target composition/adapters/contracts/lifecycle/access consumers | Cat Catch 规则与产品 policy 分层、page policy、vault/store、tokenized document-bound probe routing、renderer-safe reducer、唯一 network/per-view probe composition、owner lifecycle、四类 opaque resource consumer、bounded inspection、context-free stale owner 拒绝和 redirect hop 凭据隔离已有专项测试；production composition 实例化、probe install、四类 consumer 的生产入口、IPC/preload/renderer 与 cutover 仍待迁移和验证 |
