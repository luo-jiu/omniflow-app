...
| output integration | mixed: opaque resource authority + legacy HLS/DASH/drag/toolkit paths | external-tool、inspection、普通资源下载和 probe 资源动作已改为 main-owned opaque authority；已捕获资源的页面拖拽暂存已接入同一 authority，但 data/blob、未捕获资源和多资源 fallback 仍走旧暂存链；HLS/DASH 直拉、派生字幕 URL、process terminal、m3u8dl encoding、本地保存、ffmpeg、资料库导入与统一任务合同仍待迁移 |

## 3. 高优先级缺口

1. `enableDeepRuntimeHooks = false`。
2. 生产网络捕捉仍为 `onCompleted`；目标 adapter 已实现首字节阶段识别，但在整个 unit 就绪前不能与旧 listener 同时注册。
3. 生产 request context 的新 vault/store 已接入 target runtime；旧 request context/富 DTO 仍被 HLS/DASH、未捕获页面拖拽 fallback 和 catch toolkit 路径使用，尚未完成统一清理。
...
下一步补 page-drag authority 的 production-equivalent integration，并把 HLS/DASH 和 catch toolkit 的请求改成同一套 `tabId/resourceId` authority。证据完整后，在唯一 dispatch boundary 原子切换并删除对应旧实现；当前仍保留 data/blob 与未捕获资源的必要 fallback，不提前删除旧链。
