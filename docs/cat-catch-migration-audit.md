...
当前映射包含 7 个 cutover unit、32 项能力、192 个上游 anchor、106 个本地旧位置和 71 个唯一计划测试 ID。network event adapter、classifier/rules、page URL policy、main-only request context vault 与 main-owned resource state contract 已达到 `ported-unverified`，其余 27 项仍为 `pending`；11 个计划测试 ID 已落成 active pure behavior/contract 或 fake `webRequest` integration test，尚无已完成的 cutover unit。

## 2. 能力族

| 能力族 | 当前实现 | 已知结论 |
| --- | --- | --- |
| network capture | legacy production owner + unregistered target adapter/contracts | 规则顺序、默认值、大小判断、去重、页面 URL policy、main-only context vault、revisioned ResourceStateStore、首字节捕捉、逐跳 redirect、终态清理与 context 失效投影已有专项测试；IPC/reducer、安全 consumer 与生产 cutover 仍待迁移和验证 |
...
10. 目前 classifier/rules、page URL policy、request context、resource state 与 event adapter 有 11 个 pure contract 或 fake `webRequest` integration test ref；IPC/reducer、安全 consumer 及后续 unit 仍无实际 fixture、differential 或 production-equivalent integration test。
...
下一步实现 revisioned IPC snapshot/change reducer，以及禁用自动 redirect 或逐跳重新兑换的下载、检查、拖拽和外部工具 consumer；随后补齐 owner lifecycle 与整个 network-capture unit 的 production-equivalent integration。整个 unit 通过后，再在唯一入口切换并删除对应旧实现；当前 adapter 不接生产，也不提前删除旧链。
