...
当前映射包含 7 个 cutover unit、32 项能力、208 个上游 anchor、106 个 cleanup entry 和 177 个唯一计划测试 ID。11 项能力达到 `verified`，4 项为 `porting`，1 项为 `ported-unverified`，其余 16 项仍为 `pending`；140 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`network-capture` 与 `hls-engine` 已完成固定目标下的原子 cutover，其余 5 个 unit 仍开放。
...
| deep-search runtime | pure discovery/hook port + verified relay boundary + inactive legacy hooks | 固定 `findMedia/toUrl` 的 width/depth/cycle、宽松 key、inline M3U8/MPD、data URL 和 base URL 回放已有 upstream-executable fixture；target installer 已覆盖 Worker CSP 回退、bootstrap relay、fetch clone、XHR、JSON、TextDecoder 与 Cat Catch key/string 经验面；随机 document token、owner binding、CDP 新文档与当前 frame subtree 注入已有平台测试。inline DOM/Vimeo、adapter/cutover 尚未完成，生产 hooks 仍关闭 |
...
5. JSON 深度/宽度/cycle 的 pure discovery 已对齐；尚未通过 document-start JSON/fetch/XHR hook 接入生产。
...
10. 目前有 140 个唯一 active test ref；逐项名称与来源以 capability map 为准。Network/HLS 证据已支撑对应 unit cutover；deep/MSE/DASH/transfer/output 仍按各自开放状态判断。
...
下一步进入 `deep-search-runtime`，先恢复并验证当前关闭的 Worker/fetch/XHR/JSON/TextDecoder/manifest/key hooks，再在完整 unit 证据齐备后原子切换。Network/HLS 后续只在真实回归发现问题或上游游标前进时增量维护。
