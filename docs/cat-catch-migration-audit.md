...
当前映射包含 7 个 cutover unit、32 项能力、192 个上游 anchor、106 个本地旧位置和 77 个唯一计划测试 ID。`network-capture` 的 7 项能力均达到 `ported-unverified`，`output.external-tools-dispatch` 为 `porting`，其余 24 项仍为 `pending`；22 个计划测试 ID 已落成 active pure behavior/contract、fake Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。

## 2. 能力族

| 能力族 | 当前实现 | 已知结论 |
| --- | --- | --- |
| network capture | legacy production owner + unregistered target adapters/contracts/lifecycle/access consumer | Cat Catch 规则与产品 policy 分层、page policy、vault/store、tokenized document-bound probe routing、renderer-safe reducer、owner lifecycle、opaque resource authority、context-free stale owner 拒绝和 redirect hop 凭据隔离已有专项测试；production probe install、四类旧 consumer 的生产入口、IPC/preload/renderer 与 cutover 仍待迁移和验证 |
...
10. 目前有 22 个 active pure contract、fake Electron integration 或 loopback redirect test ID；external-tool target consumer 已有 opaque authority 证据，但 production probe install、四类旧 consumer 的生产入口、IPC/preload/renderer 及所有 unit 仍无 production cutover 证据。
...
下一步把目标 access consumer 接入下载、检查和拖拽，并让 production probe 安装使用 tokenized document ingress；随后把已完成的 external-tool target consumer 与安全合同一起接入 production IPC/preload/renderer，建立 cutover integration。证据完整后，在唯一 dispatch boundary 原子切换并删除对应旧实现；当前目标链不接生产，也不提前删除旧链。
