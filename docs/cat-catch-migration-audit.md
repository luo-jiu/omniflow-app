...
当前映射包含 7 个 cutover unit、32 项能力、210 个上游 anchor、99 个 cleanup entry 和 188 个唯一计划测试 ID。15 项能力达到 `verified`，5 项为 `porting`，1 项为 `ported-unverified`，其余 11 项仍为 `pending`；165 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成固定目标下的原子 cutover，`dash.parser-planner` 与 `dash.timeline-download-merge` 已建立 target 基座但尚未生产切换，其余 3 个 unit 仍开放。
...
| DASH engine | target parser/task porting + legacy execution owner | `cat-catch-port/dash/parser.ts` 已覆盖继承 BaseURL、模板 token、有限 `r=-1`、SegmentList range 和 DRM 投影；`processing/dash-task.ts` 已收口 range、顺序写入、取消和失败清理；生产仍使用旧 MPD dispatch/downloader，动态无界 repeat、SegmentBase SIDX、ffmpeg 取消和原子切换尚未完成 |
...
4. MPD SegmentBase/SIDX、动态 availability、多 Period 合并和真实 output 仍不完整；有限 timeline/range 已进入 target parser/task，但尚未生产切换。
5. ffmpeg、HLS/DASH、直播、普通下载和 temp 没有应用级统一 task registry；HLS 的 host lifecycle 已收口，但非 HLS 的 4 个 ffmpeg 入口仍未纳入该 owner。
6. 目前有 165 个唯一 active test ref；逐项名称与来源以 capability map 为准。Network/Deep/HLS 证据已支撑对应 unit cutover；MSE 的纯 runtime、spool 生命周期、relay 合同和 synthetic output contract 已有证据，但 production 大媒体与真实下载导入仍未完成；DASH parser-planner 与 timeline-download-merge 进入 porting，transfer/output 仍按各自开放状态判断。
...
下一步继续收口 `dash-engine`：补固定 `mpd-parser` 差分、SegmentBase/Dynamic 语义，把 `DashTaskExecutor` 接入 main-owned authority/ffmpeg 并完成 production-equivalent output，再决定原子 cutover；MSE 只补真实页面与固定上游 parity，Network/Deep/HLS 后续只在真实回归发现问题或上游游标前进时增量维护。
