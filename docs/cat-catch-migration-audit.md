...
当前映射包含 7 个 cutover unit、32 项能力、192 个上游 anchor、106 个本地旧位置和 71 个唯一计划测试 ID。network event adapter、classifier/rules、page URL policy、main-only request context vault、main-owned resource state 与 renderer-safe cross-process contract/reducer 已达到 `ported-unverified`，其余 26 项仍为 `pending`；13 个计划测试 ID 已落成 active pure behavior/contract 或 fake `webRequest` integration test，尚无已完成的 cutover unit。

## 2. 能力族

| 能力族 | 当前实现 | 已知结论 |
| --- | --- | --- |
| network capture | legacy production owner + unregistered target adapter/contracts | 规则顺序、默认值、大小判断、去重、页面 URL policy、main-only context vault、revisioned ResourceStateStore、首字节捕捉、逐跳 redirect、终态清理、context 失效投影与 renderer-safe reducer 已有专项测试；生产 IPC/preload/renderer、安全 consumer 与 cutover 仍待迁移和验证 |
...
10. 目前 classifier/rules、page URL policy、request context、resource state、cross-process reducer 与 event adapter 有 13 个 pure contract 或 fake `webRequest` integration test ref；生产 IPC/preload/renderer、安全 consumer 及后续 unit 仍无实际 fixture、differential 或 production-equivalent integration test。
...
下一步补齐 tab/navigation/WebContents/app owner lifecycle，以及禁用自动 redirect 或逐跳重新兑换的下载、检查、拖拽和外部工具 consumer；随后建立整个 network-capture unit 的 production-equivalent integration。整个 unit 通过后，再原子切换 production IPC/preload/renderer 并删除对应旧实现；当前 adapter/contract 不接生产，也不提前删除旧链。
