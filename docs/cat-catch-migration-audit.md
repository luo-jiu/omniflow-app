...
当前映射包含 7 个 cutover unit、32 项能力、208 个上游 anchor、106 个 cleanup entry 和 169 个唯一计划测试 ID。9 项能力达到 `ported-unverified`，5 项为 `porting`，其余 18 项仍为 `pending`；124 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。
...
7. HLS master/media parser 双向模式隔离、master 普通 variant/I-frame/未知 codec 过滤与无 level 拒绝、AUDIO/SUBTITLES rendition 投影与 child authority、直播及独立双轨 child 的 main-owned master 归属/变量恢复、`EXT-X-SESSION-KEY` 解析但不进入 fragment key、带值标签精确冒号/零字符与 whitespace payload 分发、无值 `DISCONTINUITY/ENDLIST` 前缀行为、整数首 token、`EXTINF` decimal-prefix 回扫、CR/LF/CRLF 行边界与行首空格/Tab token 优先级、媒体/MAP BYTERANGE offset 与数值归一化、MAP 前置独立 range 双重绑定与缺失/空 URI 拒绝、CBC 加密 MAP range 补齐与 previous-block IV reset、full-segment AES media/MAP effective IV、独立 MAP/media key context、KEYFORMAT 支持/忽略/继承/多 key 选择、正数 `EXT-X-SKIP` delta 拒绝与同 key URL 下的 playlist 状态轮换、初始/当前 discontinuity sequence 双状态、LL-HLS PART/完整分片边界、空/无效 media playlist 与重复 singleton 标签拒绝、`EXT-X-DEFINE` 变量语义、`tsAddArg` fragment-only query 替换及独立双轨本地合并、PNG/JPEG 伪装分片剔除和一次性 manifest force-cache recovery 已在 pure/integration tests 中覆盖；local-to-ffmpeg fake handoff 已验证本地 key/map/media 引用与非空输出约束，真实 ffmpeg/ffprobe integration 已验证 clear、两类 AES-128 encrypted AAC HLS、AES-256 CBC/CTR encrypted AAC HLS 的本地预解密，以及 AES-128 加密 fMP4/H264 视频与独立 AES-128/AAC 音轨均可交付为含预期流、正时长的 MP4；更完整 parser 差分和真实网站仍缺失。
...
10. 目前有 124 个唯一 active test ref；逐项名称与来源以 capability map 为准。network composition/authority/consumer、HLS contract/parser/plan/local/live owner、关键 parser 经验分支、keyless encrypted/manual-key fallback、CBC 加密 MAP range/IV、retry/cancel、生命周期清理和真实 AES-128/AES-256 CBC/CTR 输出已有专项证据，普通资源下载和 inspection 已有 production IPC 入口；legacy-named controller adapter、页面拖拽 fallback、HLS parser 其余下载语义、旧 toolkit 及完整 unit cutover 仍无 production cutover 证据。
