...
当前映射包含 7 个 cutover unit、32 项能力、210 个上游 anchor、99 个 cleanup entry 和 197 个唯一计划测试 ID。15 项能力达到 `verified`，5 项为 `porting`，1 项为 `ported-unverified`，其余 11 项仍为 `pending`；176 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成固定目标下的原子 cutover；DASH 的 renderer parser facade、main task 和 output adapter 已接入生产 dispatch，但 `dash.parser-planner` 与 `dash.timeline-download-merge` 仍保持 `porting`，其余 3 个 unit 仍开放。
...
4. MPD 动态 availability 和真实 output 仍不完整；同身份静态多 Period 已进入 target parser 的串接，但不完整/初始化冲突集合、复杂嵌套 SIDX、真实 MPD/ffprobe parity 仍未达到 unit 关闭条件。
...
下一步继续收口 `dash-engine`：补固定 `mpd-parser` 的 dynamic availability、复杂 SIDX 语义和真实 MPD/ffprobe 输出证据；DASH dispatch 已切换，同身份静态多 Period 已完成受约束串接，后续以行为差分和真实回归决定是否关闭 unit。MSE 只补真实页面与固定上游 parity，Network/Deep/HLS 后续只在真实回归发现问题或上游游标前进时增量维护。
