...
当前映射包含 7 个 cutover unit、32 项能力、194 个上游 anchor、106 个 cleanup entry 和 134 个唯一计划测试 ID。9 项能力达到 `ported-unverified`，5 项为 `porting`，其余 18 项仍为 `pending`；88 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。
