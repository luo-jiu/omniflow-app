...
| DASH engine | target parser porting + legacy execution owner | `cat-catch-port/dash/parser.ts` 已覆盖继承 BaseURL、模板 token、有限 `r=-1`、SegmentList range 和 DRM 投影；生产仍使用旧 renderer parser/downloader，动态无界 repeat、SegmentBase SIDX、下载/合并和 dispatch 尚未切换 |
...
下一步继续收口 `dash-engine`：补 parser 与固定 `mpd-parser` 的差分、SegmentBase/Dynamic 语义和 main 下载/合并执行证据，再决定 production cutover；MSE 只补真实页面与固定上游 parity，Network/Deep/HLS 后续只在真实回归发现问题或上游游标前进时增量维护。
