...
当前映射包含 7 个 cutover unit、32 项能力、211 个上游 anchor、104 个 cleanup entry 和 229 个唯一计划测试 ID。15 项能力达到 `verified`，12 项为 `porting`，1 项为 `ported-unverified`，其余 4 项仍为 `pending`；238 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test。`network-capture`、`deep-search-runtime` 与 `hls-engine` 已完成固定目标下的原子 cutover；DASH 的 renderer parser facade、main task、live task、main XML adapter、增量追加 helper、独立 session owner、strict manifest authority 和 output adapter 已接入目标代码，静态双轨、dynamic refresh/append 与有限深度嵌套 SIDX 的真实/任务级证据已有，live start/stop/discard 也已接生产 main/preload IPC（当前没有 renderer UI），并且轮询终止错误会清理 live session/workdir；native `will-download` 现由 `NativeDownloadSession` 负责 staging、进度、终态和 registry 取消，分片 downloader 也由单一外部 signal 收口并发取消；普通直链、已捕获资源、HLS/MPD 直拉、HLS 计划下载、DASH 静态计划、DASH 直播停止导出、手动 MSE 合并、手动捕获资源保存、MSE 资源导出和 MSE 自动/周期/工具触发的下载任务已进一步通过 `StagedOutputLeaseStore`、`publishStagedOutput` 或 `stageMseDownloadResource` 完成处理期隔离、任务登记、单次 claim/事件终态与失败清理；`CapturedOutputWorkflowCoordinator` 已把完成文件队列和失败清理提升到应用级，可在 library-detail listener 卸载/重挂载时保留待处理输出，但 MSE 自动导入终态、local-save/UploadManager delivery terminal、staged lease crash quarantine 和 Cat Catch fallback 仍未完整接入；`dash.parser-planner` 与 `dash.timeline-download-merge` 仍保持 `porting`，其余 3 个 unit 仍开放。
