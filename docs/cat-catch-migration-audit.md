...
当前映射包含 7 个 cutover unit、32 项能力、208 个上游 anchor、106 个 cleanup entry 和 167 个唯一计划测试 ID。9 项能力达到 `ported-unverified`，5 项为 `porting`，其余 18 项仍为 `pending`；122 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。
...
10. 目前有 122 个唯一 active test ref；逐项名称与来源以 capability map 为准。network composition/authority/consumer、HLS contract/parser/plan/local/live owner、关键 parser 经验分支、CBC 加密 MAP range/IV、retry/cancel、生命周期清理和真实 AES-128/AES-256 CBC/CTR 输出已有专项证据，普通资源下载和 inspection 已有 production IPC 入口；legacy-named controller adapter、页面拖拽 fallback、HLS parser 其余下载语义、旧 toolkit 及完整 unit cutover 仍无 production cutover 证据。
