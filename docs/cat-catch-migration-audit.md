...
当前映射包含 7 个 cutover unit、32 项能力、201 个上游 anchor、106 个 cleanup entry 和 145 个唯一计划测试 ID。9 项能力达到 `ported-unverified`，5 项为 `porting`，其余 18 项仍为 `pending`；99 个唯一计划测试引用已落成 active pure behavior/contract、fake/real Electron integration 或 loopback redirect test，尚无已完成的 cutover unit。
...
Cat Catch 的 `tsAddArg` 已按原分支顺序迁入 post-parse plan：`null` 保留 fragment URL，空字符串清除原 query，非空字符串替换原 query；key/MAP URL 始终保持。工具区从当前 manifest URL 按固定大小写敏感 `.m3u8?` 正则提供默认草稿，但只有用户显式启用才生效。静态计划因此强制走本地 downloader，master 必须先选具体 variant；直播通过显式 IPC 在每轮 snapshot 使用同一设置，任务日志不记录参数值。
