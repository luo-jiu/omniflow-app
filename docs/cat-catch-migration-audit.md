...
| HLS engine | pure parser slice + legacy pipeline + partial authority transport | 纯 port 已接管 manifest parse facade，并用 fixture 覆盖同一资源隐式 BYTERANGE offset、初始/增量 discontinuity sequence、map/key；计划下载和 live 录制的已捕获 manifest/分片/key/map 已优先走当前 tab authority；完整 parser 语义、cache fallback、伪装分片和 task owner 仍有明确缺口 |
...
7. HLS 隐式 BYTERANGE offset 已在纯 port 和 fixture 中覆盖，但完整 parser/output 差分、一次性 cache fallback、PNG/JPEG 伪装分片仍缺失。
