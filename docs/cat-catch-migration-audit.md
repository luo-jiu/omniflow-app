...
| HLS engine | pure parser slice + legacy pipeline + partial authority transport | 纯 port 已接管 manifest parse facade，并用 fixture 覆盖同一资源隐式 BYTERANGE offset、map/key/discontinuity；计划下载和 live 录制的已捕获 manifest/分片/key/map 已优先走当前 tab authority；完整 parser 语义、cache fallback、伪装分片和 task owner 仍有明确缺口 |
