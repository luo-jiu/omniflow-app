...
- 资源监测打开 `resource-monitor` system view；资料库详情页会按当前 `libraryId` 展示单资料库物理存储分布快照和可见 / 回收站 / 孤儿对象占用细分；只读资源探针按 provider / 基础设施级别展示，不按资料库过滤；可显式记录一条历史样本，可跳转到存储设置、迁移任务，且可跳转到当前资料库回收站。
