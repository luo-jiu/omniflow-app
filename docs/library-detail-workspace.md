...
表示当前工作区显示系统视图。第一版用于把设置、个人主页、上传中心、回收站等入口放回资料库详情页右侧主内容区，避免全屏覆盖打断目录树、文件 tab、浏览器 tab 和后台媒体播放。

当前规则：

- 系统视图只在 `library detail` 右侧主内容区显示，左侧目录树继续保留。
- 系统视图按类型唯一；再次打开同一视图时聚焦现有 system tab，打开不同系统视图时可以并存在 system tab 栏里。
- 系统 tab 不是文件 tab，也不是浏览器 tab，不进入 `FileViewerContext` 或浏览器 tab 列表。
...
- 资源页系统任务进入 `library detail` 的 `system` 工作区，使用 system tab。
...
- 系统视图进入后在中间区域显示 system tab 栏；同类 tab 唯一，不同系统视图可并存，可通过 `x` 关闭当前 tab。
