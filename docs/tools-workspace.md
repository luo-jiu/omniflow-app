...
- 保存已选资源：HTTP 资源走本地下载目录，probe / page-managed 资源走页内导出。
- 合并音视频：复用当前 embedded browser 的本地 ffmpeg 合并链路，完成后保存到用户选择的位置。
- 转格式：单个媒体资源可输入目标扩展名后通过本地 ffmpeg 尝试转换，完成后保存到用户选择的位置；扩展名只接受 1-12 位字母或数字，例如 `mp3`、`m4a`、`mp4`、`wav`。如果 ffmpeg 不支持该封装或编码组合，错误直接回传给用户。
