...
`AppMain` 还会把认证账号 scope 与 tab 上的 `libraryId`、`contentRevision`、`reloadToken` 作为 session identity 输入交给 `FileDispatcher`。当前 PDF 和 Text 已消费这些字段接入公共 Viewer Session Registry，Text 还用稳定账号/资料库/节点身份定位 IndexedDB draft；分发器只透传身份事实，不持有阅读状态或草稿正文。
