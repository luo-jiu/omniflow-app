...
`AppMain` 还会把认证账号 scope 与 tab 上的 `libraryId`、`contentRevision`、`reloadToken` 作为 session identity 输入交给 `FileDispatcher`。当前 PDF 已消费这些字段接入公共 Viewer Session Registry；分发器只透传身份事实，不持有 PDF 阅读状态。
