...
import { formatETA, formatRate } from '@/modules/upload-center/utils';

// 上传中心列表主体：被 transfer-center 上传 tab 复用，不再带 Page 外框 / 页头 / 标题。
// Page 外壳、标题、subtitle、返回按钮都由 views/transfer-center 的容器统一负责。
...
interface UploadGroupStats {
  ...
  canceled: number;
  latestAt: number;
  // 仅累加 status === 'uploading' 的 task speedBps；其他状态 speedBps 已在 state-machine 里被清零。
  speedBps: number;
}
...
function applyTaskStats(target: UploadGroupStats, task: UploadTask): void {
  ...
  switch (task.status) {
    ...
    case 'uploading':
      target.uploading += 1;
      target.speedBps += Math.max(0, Number(task.progress.speedBps || 0));
      break;
    case 'paused':
      target.paused += 1;
      break;
...
// 组内 ETA：剩余字节 / 当前合计速率。无上传中 task 或 speedBps 为 0 返回 null。
function getGroupEtaSeconds(stats: UploadGroupStats): number | null {
  if (stats.uploading <= 0 || stats.speedBps <= 0) return null;
...
const UploadCenterBody: React.FC = () => {
...
export default UploadCenterBody;
