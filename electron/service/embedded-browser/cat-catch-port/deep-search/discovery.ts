...
export type DeepSearchDiscoverySession = {
  discover: (input: unknown, options?: DeepSearchDiscoveryOptions) => DeepSearchDiscovery[]
}

export function createDeepSearchDiscoverySession(
  options: DeepSearchDiscoveryOptions,
): DeepSearchDiscoverySession {
  ...
  let currentDiscoveries: DeepSearchDiscovery[] = []
  let protocol = 'https:'

  const getProtocol = (pageUrl: string) => {
    try {
  ...
  const isCatCatchUrl = (input: string) => input.startsWith('http://')
  ...
  const applyProtocolToSegments = (text: string) => {
  ...
  const hasFullSegmentUrl = (text: string) => {
  ...
  const addBaseUrl = (baseUrl: string, text: string) => {
    let output = ''
  ...
  const decodeBase64 = (input: string) => (
    typeof globalThis.atob === 'function' ? globalThis.atob(input) : ''
  )
  const decodeDataManifest = (input: string) => {
  ...
  const isLooseCatCatchKey = (input: unknown): input is number[] => Array.isArray(input)
    && input.length === 16
    && input.every(value => typeof value === 'number' && value <= 256)
  const arrayToBase64 = (input: ArrayBuffer | number[]) => {
  ...
  const emitInline = (text: string, ext: 'm3u8' | 'mpd') => {
    currentDiscoveries.push({ ext, kind: 'inline', text })
  }
  const emitMedia = (url: string, ext: string) => {
    if (emittedValues.has(url)) return
    emittedValues.add(url)
    currentDiscoveries.push({ ext, kind: 'media', url })
  }
  const emitKey = (value: ArrayBuffer | number[]) => {
    ...
    currentDiscoveries.push({ base64, ext: 'key', kind: 'key' })
  }
  ...
  const emitManifest = (value: string, ext: 'm3u8' | 'mpd' = 'm3u8') => {
    if (!value) return
    if (ext === 'mpd') {
      emitInline(value, ext)
      return
    }
    const normalized = applyProtocolToSegments(value)
    if (hasFullSegmentUrl(normalized)) {
      emitInline(normalized, ext)
      return
    }
    for (const baseUrl of baseUrls) emitInline(addBaseUrl(baseUrl, normalized), ext)
  ...
  const visit = (value: unknown, depth = 0) => {
    ...
    for (const key in Object(value)) {
      ...
      if (isCatCatchUrl(nestedValue)) {
        const extension = getExtension(nestedValue)
        if (extension) {
  ...
  protocol = getProtocol(options.pageUrl)
  currentDiscoveries = []
  rememberBaseUrl(options.pageUrl)
  currentDiscoveries = []

  return {
    discover(input, nextOptions) {
      currentDiscoveries = []
      if (nextOptions?.pageUrl) rememberBaseUrl(nextOptions.pageUrl)
      visit(input)
...
export function createDeepSearchDiscoverySessionSource() {
  return `(${createDeepSearchDiscoverySession.toString()})`
}
...
export function discoverResources(
  ...
  return createDeepSearchDiscoverySession(options).discover(input)
}
