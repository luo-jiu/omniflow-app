...
export type DeepSearchPageDiscovery = {
...
export function createDeepSearchPageDiscovery(): DeepSearchPageDiscovery {
  const vimeoPlaylistPattern = /^https:\/\/[^.]*\.vimeocdn\.com\/exp=.*\/playlist\.json\?/i
  const inlineMediaPattern = /["']((?:(?:https?:)?\/\/)?[^"'\s]*?\.(?:m3u8|mp4|flv)(?:\?[^"'\s]*)?)["']/gi
  const isCatCatchUrl = (value: string) => value.startsWith('http://')
    || value.startsWith('https://')
    || value.startsWith('//')
  const parsePlaylist = (input: unknown): VimeoPlaylist | undefined => {
  ...
  const extractInlineScriptMediaCandidates = (
    scriptTexts: Iterable<string>,
    protocol: string,
  ) => {
...
export function createDeepSearchPageDiscoverySource() {
  return `(${createDeepSearchPageDiscovery.toString()})`
}
...
export function extractInlineScriptMediaCandidates(
  ...
  return createDeepSearchPageDiscovery().extractInlineScriptMediaCandidates(scriptTexts, protocol)
}
...
export function buildVimeoHlsManifest(
  ...
  return createDeepSearchPageDiscovery().buildVimeoHlsManifest(
    originalUrl,
    input,
    materializeManifest,
  )
