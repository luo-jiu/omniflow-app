...
- `dash/parser.ts`：固定 MPD 的继承 BaseURL、Period/AdaptationSet/Representation segment info、SegmentTemplate/SegmentTimeline、SegmentBase SIDX metadata、SegmentList range 与 DRM 投影语义；XML DOM 由平台 adapter 注入。`dash/sidx.ts` 负责 main task 下载 index range 后的纯 SIDX reference expansion。
