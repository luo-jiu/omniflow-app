...
const MAC_CHROME_BOOKMARK_RELATIVE_PATH = path$n.join(
  "Library",
  "Application Support",
  "Google",
  "Chrome",
...
function getAutoImportStagingRoot() {
  return path$n.join(app.getPath("userData"), "auto-import-staging");
}
function getEmbeddedBrowserDownloadStagingRoot() {
  return path$n.join(app.getPath("userData"), "embedded-browser-downloads");
}
function getTextFileStagingRoot() {
  return path$n.join(app.getPath("userData"), "text-file-staging");
}
function getTempImportStagingRoot() {
  return path$n.join(app.getPath("temp"), TEMP_IMPORT_STAGING_DIR_NAME);
}
...
function isPathInsideDirectory$1(filePath, directoryPath) {
  ...
  const resolvedDirectoryPath = path$n.resolve(directoryPath);
  if (resolvedFilePath === resolvedDirectoryPath) return true;
  return resolvedFilePath.startsWith(`${resolvedDirectoryPath}${path$n.sep}`);
}
...
async function moveFileSafe(sourcePath, targetPath) {
  try {
    await fs$l.rename(sourcePath, targetPath);
  } catch (error2) {
    if ((error2 == null ? void 0 : error2.code) !== "EXDEV") {
      throw error2;
    }
    await fs$l.copyFile(sourcePath, targetPath);
    await fs$l.rm(sourcePath, { force: true });
  }
}
...
async function claimStableInboxFiles(watchDirectory, maxFiles = AUTO_IMPORT_DEFAULT_MAX_FILES) {
  ...
  const normalizedDirectory = rawDirectory ? path$n.resolve(rawDirectory) : path$n.join(app.getPath("downloads"), AUTO_IMPORT_DEFAULT_DIR_NAME);
  const stat2 = await fs$l.stat(normalizedDirectory).catch(() => null);
  if (!(stat2 == null ? void 0 : stat2.isDirectory())) {
    return [];
  }
  const entries = await fs$l.readdir(normalizedDirectory, { withFileTypes: true });
  ...
  for (const entry of entries) {
    ...
    const sourcePath = path$n.join(normalizedDirectory, entry.name);
    const fileStat = await fs$l.stat(sourcePath).catch(() => null);
  ...
  await fs$l.mkdir(stagingRoot, { recursive: true });
  const claimedFiles = [];
  ...
  for (const candidate of readyCandidates.slice(0, claimLimit)) {
    const stagedPath = path$n.join(stagingRoot, buildStagedFileName$1(candidate.name));
    try {
...
async function cleanupStagedFile(stagedPath) {
  ...
  await fs$l.rm(normalizedPath, { force: true });
  return true;
}
function resolveTargetPath(baseDirectory, relativePath) {
  ...
  return path$n.join(baseDirectory, ...segments);
}
...
async function collectFilesFromSelectedFilePaths(filePaths) {
  const files = await Promise.all(filePaths.map(async (filePath) => {
    ...
    const fileName = path$n.basename(filePath);
...
async function walkDirectoryFiles(rootPath, currentPath, rootDisplayName) {
  ...
  while (pendingDirectories.length > 0) {
    ...
    const entries = await fs$l.readdir(directoryPath, { withFileTypes: true });
    for (const entry of entries) {
      ...
      const absolutePath = path$n.join(directoryPath, entry.name);
  ...
  const statWorker = async () => {
    while (currentIndex < pendingFiles.length) {
      ...
      const stat2 = await fs$l.stat(candidate.absolutePath).catch(() => null);
      if (!(stat2 == null ? void 0 : stat2.isFile())) {
        continue;
      }
      const relativeInsideRoot = normalizeRelativePath(path$n.relative(rootPath, candidate.absolutePath));
      const relativePath = normalizeRelativePath(path$n.join(rootDisplayName, relativeInsideRoot));
      files.push({
        name: candidate.name,
        size: stat2.size,
...
async function collectFilesFromSelectedFolders(folderPaths) {
  ...
  for (const folderPath of folderPaths) {
    const folderStat = await fs$l.stat(folderPath);
    if (!folderStat.isDirectory()) {
      continue;
    }
    const folderName = path$n.basename(folderPath);
...
function registerFileIpc(ipcMain2) {
  ipcMain2.handle("file:open", async (_event, options) => {
    ...
    return {
      canceled: false,
      content: await fs$l.readFile(filePath, "utf-8"),
      filePath
    };
  });
  ipcMain2.handle("file:save", async (_e, filePath, content) => {
    await fs$l.writeFile(filePath, content, "utf-8");
    return true;
  });
  ipcMain2.handle("fs:write-text-file", async (_event, filePath, content) => {
    const normalizedPath = path$n.resolve(String(filePath || "").trim());
    if (!normalizedPath) {
      throw new Error("无效的文本保存路径");
    }
    await fs$l.mkdir(path$n.dirname(normalizedPath), { recursive: true });
    await fs$l.writeFile(normalizedPath, String(content ?? ""), "utf-8");
  ...
  ipcMain2.handle("file:read-text", async (_event, filePath) => {
    ...
    return {
      canceled: false,
      content: await fs$l.readFile(normalizedPath, "utf-8"),
      filePath: normalizedPath
    };
  });
  ipcMain2.handle("file:read-local-chrome-bookmarks", async () => {
    const filePath = path$n.join(app.getPath("home"), MAC_CHROME_BOOKMARK_RELATIVE_PATH);
    return {
      canceled: false,
      content: await fs$l.readFile(filePath, "utf-8"),
      filePath
  ...
  ipcMain2.handle("fs:ensure-directory", async (_event, baseDirectory, relativePath = "") => {
    ...
    await fs$l.mkdir(targetPath, { recursive: true });
    return targetPath;
  });
  ...
  ipcMain2.handle("fs:save-staged-download-file", async (_event, stagedPath, targetFilePath) => {
    const normalizedSourcePath = path$n.resolve(String(stagedPath || "").trim());
    const normalizedTargetPath = path$n.resolve(String(targetFilePath || "").trim());
    ...
    await fs$l.mkdir(path$n.dirname(normalizedTargetPath), { recursive: true });
    await fs$l.copyFile(normalizedSourcePath, normalizedTargetPath);
    return normalizedTargetPath;
  });
  ipcMain2.handle("fs:create-staged-text-file", async (_event, fileName, content) => {
    const stagingRoot = getTextFileStagingRoot();
    const subDir = path$n.join(stagingRoot, `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    await fs$l.mkdir(subDir, { recursive: true });
    const safeName = String(fileName || "subtitle.txt").replace(/[/\\]/g, "_").trim() || "unknown";
    const stagedPath = path$n.join(subDir, safeName);
    const normalizedContent = String(content ?? "");
    await fs$l.writeFile(stagedPath, normalizedContent, "utf-8");
    return {
      filePath: stagedPath,
  ...
  ipcMain2.handle("fs:create-staged-binary-file", async (_event, fileName, base64) => {
    ...
    await fs$l.writeFile(stagedPath, buffer);
    return {
      filePath: stagedPath,
      size: buffer.length
    };
  ...
  ipcMain2.handle("fs:create-temp-import-directory", async () => {
    const stagingRoot = getTempImportStagingRoot();
    await fs$l.mkdir(stagingRoot, { recursive: true });
    return await fs$l.mkdtemp(path$n.join(stagingRoot, "job-"));
  });
  ipcMain2.handle("fs:get-temp-import-file-info", async (_event, filePath) => {
    ...
    const stat2 = await fs$l.stat(normalizedPath);
    if (!stat2.isFile()) {
      throw new Error("临时导入路径不是文件");
    }
    return {
      filePath: normalizedPath,
      name: path$n.basename(normalizedPath),
      size: Number(stat2.size || 0)
    };
  });
  ipcMain2.handle("fs:cleanup-staged-text-file", async (_event, stagedPath) => {
    ...
    const parentDir = path$n.dirname(normalizedPath);
    if (parentDir !== stagingRoot && isPathInsideDirectory$1(parentDir, stagingRoot)) {
      await fs$l.rm(parentDir, { recursive: true, force: true });
    } else {
      await fs$l.rm(normalizedPath, { force: true });
    }
    return true;
  ...
  ipcMain2.handle("fs:cleanup-temp-import-path", async (_event, targetPath) => {
    const normalizedPath = path$n.resolve(String(targetPath || "").trim());
    const stagingRoot = getTempImportStagingRoot();
    if (!normalizedPath || !isPathInsideDirectory$1(normalizedPath, stagingRoot)) {
      return false;
    }
    const stat2 = await fs$l.stat(normalizedPath).catch(() => null);
    if (stat2 == null ? void 0 : stat2.isFile()) {
      const parentDir = path$n.dirname(normalizedPath);
      if (parentDir !== stagingRoot && isPathInsideDirectory$1(parentDir, stagingRoot)) {
        await fs$l.rm(parentDir, { recursive: true, force: true });
    ...
    await fs$l.rm(normalizedPath, { force: true, recursive: true });
    return true;
  });
}
...
osutils.freeCommand = function(callback) {
  require$$1.exec("free -m", function(error2, stdout, stderr) {
...
osutils.harddrive = function(callback) {
  require$$1.exec("df -k", function(error2, stdout, stderr) {
    var total = 0;
    var used = 0;
    var free = 0;
    var lines = stdout.split("\n");
...
osutils.getProcesses = function(nProcess, callback) {
  ...
  require$$1.exec(command, function(error2, stdout, stderr) {
    var lines = stdout.split("\n");
    lines.shift();
    lines.pop();
    var result = "";
...
function getStorageData() {
  const stats = require$$1$1.statfsSync(process.platform === "win32" ? "C:" : "/");
...
function registerHttpIpc(ipcMain2) {
  ...
  ipcMain2.handle("http:upload:formdata", async (_event, url, filePath, formDataParams = {}, headers = {}, uploadId) => {
    return new Promise((resolve, reject) => {
      let stat2;
      try {
        stat2 = fs$k.statSync(filePath);
      } catch (error2) {
        reject(new Error(`读取上传文件失败: ${filePath} (${String(error2)})`));
        return;
      }
      if (!stat2.isFile()) {
...
--${boundary}--\r
`;
      const contentLength = Buffer.byteLength(fieldsPrefix) + Buffer.byteLength(filePrefix) + stat2.size + Buffer.byteLength(fileSuffix);
      const finalHeaders = {
        ...headers,
      ...
      const fileStream = fs$k.createReadStream(filePath, { highWaterMark: 1024 * 1024 });
  ...
  ipcMain2.handle("http:upload:presigned-put", async (event, args) => {
    ...
    try {
      ...
      throw new Error(`读取上传文件失败: ${filePath} (${String(error2)})`);
    }
    if (!stat2.isFile()) {
      throw new Error(`上传目标不是文件: ${filePath}`);
    }
    if (byteOffset + byteLength > stat2.size) {
      throw new Error(`分片越界: offset=${byteOffset}, length=${byteLength}, fileSize=${stat2.size}`);
    }
    return new Promise((resolve, reject) => {
      ...
      const fileStream = fs$k.createReadStream(filePath, {
        start: byteOffset,
...
async function canExecuteFile(candidatePath) {
  ...
  try {
    await access(candidatePath, constants$3.X_OK);
    return true;
  } catch {
    return false;
  }
...
async function createEmbeddedBrowserResourceMergeTempDir() {
  return mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-resource-merge-"));
}
...
async function resolveUniqueOutputPath(outputDirectoryPath, fileName) {
  const parsed = path$n.parse(fileName);
  for (let index = 0; index < 1e3; index += 1) {
    const suffix = index === 0 ? "" : ` (${index})`;
    const candidate = path$n.join(outputDirectoryPath, `${parsed.name}${suffix}${parsed.ext}`);
    try {
  ...
  return path$n.join(outputDirectoryPath, `${parsed.name}-${Date.now()}${parsed.ext}`);
}
...
async function processMediaToolFile(request) {
  ...
  const outputDirectoryPath = path$n.resolve(
    String(request.outputDirectoryPath || "").trim() || app.getPath("downloads")
  );
  await mkdir$4(outputDirectoryPath, { recursive: true });
...
function getCacheRoot() {
  return path$n.join(app.getPath("userData"), CACHE_DIR_NAME);
}
function getCachePaths(cacheKey) {
  const root2 = getCacheRoot();
  return {
    inputPath: path$n.join(root2, `${cacheKey}.source.heic`),
    metadataPath: path$n.join(root2, `${cacheKey}.json`),
    previewPath: path$n.join(root2, `${cacheKey}.png`)
  };
}
...
function registerImagePreviewProtocol() {
  ...
  protocol.handle(IMAGE_PREVIEW_PROTOCOL, async (request) => {
    ...
    try {
      const previewBuffer = await fs$j.readFile(previewPath);
...
async function readCachedResult(cacheKey) {
  ...
  const [metadataRaw, hasPreview] = await Promise.all([
    fs$j.readFile(metadataPath, "utf-8").catch(() => ""),
    fs$j.access(previewPath).then(() => true).catch(() => false)
  ]);
...
function registerImagePreviewIpc(ipcMain2) {
  ipcMain2.handle("image-preview:prepare", async (_event, payload) => {
    ...
    await fs$j.mkdir(path$n.dirname(paths.previewPath), { recursive: true });
    try {
      await downloadUrlToFile(url, paths.inputPath);
      const inputStat = await fs$j.stat(paths.inputPath).catch(() => null);
      ...
      await fs$j.writeFile(paths.metadataPath, JSON.stringify({
        cacheKey,
      ...
      return {
        ok: false,
        error: (error2 == null ? void 0 : error2.message) || "生成 HEIC 预览失败",
        metadataRows: []
      };
    } finally {
      await fs$j.rm(paths.inputPath, { force: true }).catch(() => void 0);
    }
  });
}
...
function getEmbeddedBrowserDownloadRoot() {
  return path$n.join(app.getPath("userData"), EMBEDDED_BROWSER_DOWNLOAD_DIRNAME);
}
function ensureEmbeddedBrowserDownloadRoot() {
  const root2 = getEmbeddedBrowserDownloadRoot();
  if (!existsSync(root2)) {
...
async function cleanupEmbeddedBrowserDownloadFile(tempPath) {
  const normalizedPath = path$n.resolve(String(tempPath || "").trim());
  if (!normalizedPath) {
    return false;
  }
  const downloadRoot = path$n.resolve(getEmbeddedBrowserDownloadRoot());
  if (normalizedPath !== downloadRoot && !normalizedPath.startsWith(`${downloadRoot}${path$n.sep}`)) {
    return false;
  }
...
function initializeEmbeddedBrowserDownloadBridge(options) {
  ...
  const handleWillDownload = (_event, item, webContents2) => {
    ...
    const tempPath = path$n.join(downloadRoot, buildStagedDownloadName(fileName));
...
function getStorePath() {
  return path$n.join(app.getPath("userData"), STORE_FILE_NAME$2);
}
function loadPasswordStore() {
  ...
  try {
    const raw = readFileSync$1(storePath, "utf-8");
...
function savePasswordStore(store) {
  cachedStore = store;
  const storePath = getStorePath();
  const storeDir = path$n.dirname(storePath);
  if (!existsSync(storeDir)) {
    mkdirSync(storeDir, { recursive: true });
  }
  writeFileSync$1(storePath, JSON.stringify(store, null, 2), "utf-8");
}
...
function getRuleStorePath() {
  return path$n.join(app.getPath("userData"), STORE_FILE_NAME$1);
}
...
function normalizeRuleSet(input) {
  const defaults2 = createDefaultRuleSet();
  ...
  const extensions = Array.from(/* @__PURE__ */ new Set([
    ...((input == null ? void 0 : input.extensions) || defaults2.extensions).map(normalizeExtension).filter(Boolean),
  ...
  const mimeTypes = Array.from(/* @__PURE__ */ new Set([
    ...((input == null ? void 0 : input.mimeTypes) || defaults2.mimeTypes).map(normalizeMimeTypePattern).filter(Boolean),
    ...shouldAppendNewDefaults ? catCatchSubtitleMimeTypes.map(normalizeMimeTypePattern) : []
  ]));
  const regexRules = Array.isArray(input == null ? void 0 : input.regexRules) ? input == null ? void 0 : input.regexRules.map(normalizeRegexRule).filter(Boolean) : defaults2.regexRules;
  return {
...
function saveStoredRuleSet(ruleSet) {
  ...
  writeFileSync$1(storePath, JSON.stringify(ruleSet, null, 2), "utf-8");
}
...
function ensureEmbeddedBrowserOpenFileRoot() {
  const root2 = getEmbeddedBrowserOpenFileRoot();
  if (!existsSync(root2)) {
    mkdirSync(root2, { recursive: true });
  }
  return root2;
...
function getExternalToolStorePath() {
  return path$n.join(app.getPath("userData"), STORE_FILE_NAME);
}
...
function normalizeSettings(input) {
  ...
  const defaults2 = createDefaultEmbeddedBrowserExternalToolSettings();
  return {
    aria2: {
      ...
      label: sanitizeLabel(((_c = input == null ? void 0 : input.aria2) == null ? void 0 : _c.label) || "", defaults2.aria2.label),
      rpcUrl: String(((_d = input == null ? void 0 : input.aria2) == null ? void 0 : _d.rpcUrl) || defaults2.aria2.rpcUrl).trim(),
    ...
    command: {
      ...
      label: sanitizeLabel(((_g = input == null ? void 0 : input.command) == null ? void 0 : _g.label) || "", defaults2.command.label),
      template: String(((_h = input == null ? void 0 : input.command) == null ? void 0 : _h.template) || defaults2.command.template).trim(),
    ...
    protocol: {
      ...
      label: sanitizeLabel(((_k = input == null ? void 0 : input.protocol) == null ? void 0 : _k.label) || "", defaults2.protocol.label),
      urlTemplate: String(((_l = input == null ? void 0 : input.protocol) == null ? void 0 : _l.urlTemplate) || defaults2.protocol.urlTemplate).trim()
    }
  };
}
function loadSettingsFromDisk() {
  ...
  try {
    const raw = readFileSync$1(storePath, "utf8");
    const parsed = JSON.parse(raw);
...
function persistSettings(settings) {
  const storePath = getExternalToolStorePath();
  const directoryPath = path$n.dirname(storePath);
  if (!existsSync(directoryPath)) {
    mkdirSync(directoryPath, { recursive: true });
  }
  writeFileSync$1(storePath, JSON.stringify(settings, null, 2), "utf8");
}
...
function resolveDownloadDirectory(preferredPath) {
  return String(preferredPath || "").trim() || path$n.join(os$1.homedir(), "Downloads");
}
...
async function dispatchToCommand(settings, input) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.once("error", (error2) => {
      rejectLaunch(error2 instanceof Error ? error2 : new Error(String(error2)));
    });
...
function resetEmbeddedBrowserExternalToolSettings() {
  const defaults2 = createDefaultEmbeddedBrowserExternalToolSettings();
  cachedSettings = defaults2;
  persistSettings(defaults2);
  return cloneEmbeddedBrowserExternalToolSettings(defaults2);
}
...
function embeddedBrowserResourceProbePageActionsBody() {
  ...
  function bindTrackedMediaElements() {
    ...
    document.querySelectorAll("video, audio").forEach((node2) => {
      if (node2 instanceof HTMLMediaElement) {
        attachTrackedMediaElement(node2);
      }
    });
  }
    ...
    trackedMediaObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node2) => {
          ...
          node2.querySelectorAll("video, audio").forEach((childNode) => {
    ...
    mediaSourceConstructor.prototype.addSourceBuffer = new Proxy(originalAddSourceBuffer, {
      apply(target, thisArg, argumentsList) {
        ...
        try {
          ...
          probeDiagnostics.lastError = error2 instanceof Error ? error2.message : String(error2);
        }
        return sourceBuffer;
      }
    });
...
function isPrimaryModifierPressed(input, platform2) {
  return platform2 === "darwin" ? input.meta : input.control;
}
function isDevToolsToggleShortcut(input, platform2 = process.platform) {
  if (!isKeyDown(input)) {
    return false;
  }
...
function getZoomShortcutAction(input, platform2) {
  if (!isKeyDown(input) || !isPrimaryModifierPressed(input, platform2)) {
    return null;
  }
  const key = (input.key || "").toLowerCase();
  const code = input.code || "";
...
function getEmbeddedBrowserInputShortcutAction(input, platform2 = process.platform) {
  if (isDevToolsToggleShortcut(input, platform2)) {
    return "devtools";
  }
  return getZoomShortcutAction(input, platform2);
}
...
function handleEmbeddedBrowserInputShortcut(webContents2, input, platform2 = process.platform) {
  if (webContents2.isDestroyed()) {
    return false;
  }
  const action = getEmbeddedBrowserInputShortcutAction(input, platform2);
  if (!action) {
    return false;
  }
  if (action === "devtools") {
...
function createEmbeddedBrowserView(options) {
  ...
  const loadPopupUrlInCurrentTab = (url, details) => {
    ...
    void view.webContents.loadURL(normalizedUrl).catch((error2) => {
...
async function saveEmbeddedBrowserExtractedResourceFile(resource, outputPath) {
  if (resource.filePath) {
    await copyFile$2(resource.filePath, outputPath);
    return outputPath;
  }
  if (!resource.base64) {
    throw new Error("缺少可保存的资源内容");
  }
  await writeFile$1(outputPath, Buffer$1.from(resource.base64, "base64"));
  return outputPath;
}
...
function deriveEmbeddedBrowserManifestOutputFileName(input, kind) {
  try {
    ...
    const fileName = decodeURIComponent(path$n.basename(new URL(input).pathname)).replace(extensionPattern, "").replace(/[\\/:*?"<>|]+/g, "_").trim();
    if (fileName) {
      return `${fileName}.mp4`;
    }
  } catch {
...
function inferExtensionFromUrl(input, fallback) {
  try {
    const extension = path$n.extname(new URL(input).pathname || "").replace(/^\./, "").trim();
    return extension || fallback;
  } catch {
    const extension = path$n.extname(String(input || "")).replace(/^\./, "").trim();
    return extension || fallback;
  }
}
...
async function downloadStaticResource(input) {
  ...
  const buffer = await response.arrayBuffer();
  await writeFile$1(input.outputPath, new Uint8Array(buffer));
}
async function prepareStaticRefs(input) {
  const records = /* @__PURE__ */ new Map();
  await mkdir$4(path$n.join(input.outputDirectoryPath, input.directoryName), {
    recursive: true
  });
  let resourceIndex = 0;
...
async function prepareKeyRefs(input) {
  const records = /* @__PURE__ */ new Map();
  const keysDirectoryPath = path$n.join(input.outputDirectoryPath, "keys");
  await mkdir$4(keysDirectoryPath, { recursive: true });
  ...
  for (const ref of input.refs) {
    ...
    resourceIndex += 1;
    const outputPath = path$n.join(keysDirectoryPath, fileName);
    if (manualKeyBytes && normalizedMethod === "AES-128") {
      await writeFile$1(outputPath, manualKeyBytes);
    } else if (ref.url) {
      await downloadStaticResource({
        headers: input.headers,
        outputPath,
    ...
    records.set(cacheKey, {
      localPath: path$n.posix.join("keys", fileName),
      playlistPath: path$n.posix.join("keys", fileName)
    });
  }
  return records;
}
...
async function filterExistingPlaylistFragments(input) {
  const existence = await Promise.all(input.fragmentPaths.map(async (relativePath) => {
    try {
      await access(path$n.join(input.outputDirectoryPath, relativePath));
      return true;
    } catch {
      return false;
    }
...
async function downloadEmbeddedBrowserHlsToLocalWorkDirectory(request) {
  ...
  const outputDirectoryPath = request.workDirectoryPath ? path$n.resolve(request.workDirectoryPath) : request.outputDirectoryPath ? path$n.resolve(request.outputDirectoryPath) : await mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-hls-download-"));
  const segmentsDirectoryPath = path$n.join(outputDirectoryPath, "segments");
  await mkdir$4(segmentsDirectoryPath, { recursive: true });
  ...
  const mapRefs = await prepareStaticRefs({
    ...
    resourcePathBuilder: (index, ref) => {
      ...
      return {
        localPath: path$n.posix.join("maps", fileName),
        outputPath: path$n.join(outputDirectoryPath, "maps", fileName)
      };
    }
  });
  const fragmentPaths = plan.fragments.map((fragment, index) => {
    ...
    return path$n.posix.join("segments", fileName);
  });
  ...
  const initialCompletedFragments = requestedFragmentIndexes ? (await Promise.all(fragmentPaths.map(async (relativePath, index) => {
    ...
    try {
      await access(path$n.join(outputDirectoryPath, relativePath));
      return 1;
    } catch {
      return 0;
    }
  ...
  downloader.on("downloadError", (fragment, error2, attempt) => {
    var _a3;
    const sourceIndex = Math.max(0, getFragmentSourceIndex(fragment));
    (_a3 = request.onEvent) == null ? void 0 : _a3.call(request, {
      completedFragments: initialCompletedFragments + downloader.success,
      error: error2.message,
      message: `分片 #${sourceIndex + 1} 第 ${attempt} 次下载失败：${error2.message}`,
      stage: "downloading-fragments",
      status: "running",
      totalFragments: plan.fragments.length
    });
    ...
    downloadErrorMessage = `下载分片失败：#${sourceIndex + 1} ${error2.message}`;
  ...
  downloader.on("sequentialPush", (buffer, fragment) => {
    ...
    pendingWrites.push({
      promise: writeFile$1(
        path$n.join(outputDirectoryPath, relativePath),
  ...
  const playlistPath = path$n.join(outputDirectoryPath, "local-playlist.m3u8");
  await writeFile$1(playlistPath, playlistText, "utf8");
...
function buildMpdFaststartArgs(outputPath) {
  const normalizedExtension = path$n.extname(String(outputPath || "")).trim().toLowerCase();
...
function inferMpdRepresentationExtension(representation, fallback) {
  ...
  for (const candidate of candidates) {
    try {
      const extension = sanitizeMpdTempSegmentExtension(path$n.extname(new URL(String(candidate || "")).pathname));
      if (extension) {
        return extension;
      }
    } catch {
...
async function downloadMpdRepresentationToFile(input) {
  ...
  await writeFile$1(input.outputPath, Buffer$1.alloc(0));
  const downloader = new EmbeddedBrowserFragmentDownloader({
    fragments,
    headers: input.headers,
    maxRetries: 2,
...
async function downloadEmbeddedBrowserMpdToOutput(input) {
  ...
  const tempDirectory = await mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-mpd-download-"));
  try {
    const videoTrackPath = input.selectedVideoRepresentation ? path$n.join(
      tempDirectory,
      `video-track.${inferMpdRepresentationExtension(input.selectedVideoRepresentation, "mp4")}`
    ) : void 0;
    const audioTrackPath = input.selectedAudioRepresentation ? path$n.join(
      tempDirectory,
...
function parseEmbeddedBrowserHlsManifest(input) {
  ...
  function rememberMap(map2) {
    var _a2;
    maps.set(`${map2.url}:${((_a2 = map2.byteRange) == null ? void 0 : _a2.raw) || ""}`, map2);
  }
  ...
  lines.forEach((line) => {
    ...
    if (line.startsWith("#EXT-X-MAP")) {
      const map2 = createHlsMap(line, baseUrl);
      if (map2) {
        currentMap = map2;
        rememberMap(map2);
      }
...
class EmbeddedBrowserHlsLiveRecorder {
  ...
  async start() {
    ...
    this.workDirectoryPath = this.workDirectoryPath || await mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-hls-live-"));
  ...
  scheduleNextPoll() {
    ...
    this.pollTimer = setTimeout(() => {
      this.activePollPromise = this.pollOnce(false).catch((error2) => {
        var _a2, _b, _c, _d;
...
function createEmbeddedBrowserMainController(options) {
  ...
  async function clearEmbeddedBrowserMseSpoolFiles(options2) {
    ...
    const matchedEntries = Array.from(embeddedBrowserMseSpoolFiles.entries()).filter(([, file2]) => {
      if (normalizedTabId && file2.tabId !== normalizedTabId) {
        return false;
      }
      if (normalizedResourceKey && file2.resourceKey !== normalizedResourceKey) {
        return false;
      }
      return true;
    });
    await Promise.all(matchedEntries.map(async ([key, file2]) => {
      embeddedBrowserMseSpoolFiles.delete(key);
      embeddedBrowserMseSpoolWriteQueues.delete(key);
      await rm(file2.directoryPath, { force: true, recursive: true }).catch(() => void 0);
    }));
  }
  ...
  async function appendEmbeddedBrowserMseSpoolChunk(tabId, payload) {
    ...
    const nextWrite = (embeddedBrowserMseSpoolWriteQueues.get(spoolKey) || Promise.resolve(null)).then(async (existingSpoolFile) => {
      ...
      if (!spoolFile2) {
        const directoryPath = await mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-mse-spool-"));
        ...
        spoolFile2 = {
          ...
          directoryPath,
          fileName,
          filePath: path$n.join(directoryPath, fileName),
          mimeType: payload.mimeType,
  ...
  async function deriveEmbeddedBrowserPreferredOutputPath(directoryPath, fileName) {
    const normalizedDirectory = path$n.resolve(String(directoryPath || "").trim());
    if (!normalizedDirectory) {
      throw new Error("无效的输出目录");
    }
    await mkdir$4(normalizedDirectory, { recursive: true });
    const parsedName = path$n.parse(sanitizeEmbeddedBrowserOutputFileName(fileName));
    ...
    for (let attempt = 0; attempt < 5e3; attempt += 1) {
      ...
      const candidatePath = path$n.join(normalizedDirectory, `${baseName}${suffix}${extension}`);
      const exists = await access(candidatePath).then(() => true).catch(() => false);
      if (!exists) {
        return candidatePath;
      }
    }
    return path$n.join(normalizedDirectory, `${baseName}-${Date.now()}${extension}`);
  }
  async function resolveEmbeddedBrowserOutputPath(payload) {
    ...
    const saveDialogOptions = {
      defaultPath: path$n.join(app.getPath("downloads"), defaultFileName),
  ...
  function deriveEmbeddedBrowserDirectFileName(url, fallbackName) {
    try {
      const fileName = decodeURIComponent(path$n.basename(new URL(url).pathname)).trim();
      if (fileName) {
  ...
  async function downloadEmbeddedBrowserDirectFile(_tabId, payload) {
    ...
    try {
      ...
      await writeFile$1(outputPath, buffer);
      return {
        ok: true,
        outputPath
      };
  ...
  async function mergeEmbeddedBrowserCapturedMseResources(tabId, payload) {
    ...
    const createPayloadMergeResource = (input, fallbackStreamType) => {
      ...
      if (!fileName) {
        try {
          fileName = decodeURIComponent(path$n.basename(new URL(url).pathname));
        } catch {
          fileName = "";
        }
      }
  ...
  function deriveTranscodedFileName(fileName, outputFormat) {
    const parsedName = path$n.parse(String(fileName || "").trim() || "media");
  ...
  async function downloadEmbeddedBrowserHlsPlanResource(tabId, payload) {
    ...
    try {
      ...
      workDirectoryPath = await mkdtemp(path$n.join(os$1.tmpdir(), "omniflow-hls-download-"));
      const localDownloadResult = await downloadEmbeddedBrowserHlsToLocalWorkDirectory({
        onEvent: (event) => {
          var _a2;
  ...
  async function handleReadResource(tabId, resourceKey) {
    return withEmbeddedBrowserView(tabId, async (view) => {
      try {
        ...
        const fileBuffer = await readFile$1(resource.filePath);
        return {
...
function createOverlayWindowController(options) {
  ...
  function ensureCreated() {
    ...
    if (options.devServerUrl) {
      ...
      void win.loadFile(path$n.join(options.rendererDist, "overlay.html"));
    }
    win.on("closed", () => {
      if (overlayWin === win) {
        overlayWin = null;
...
function createSystemVideoWindowController(options) {
  ...
  function ensureCreated() {
    ...
    win.on("closed", () => {
      ...
      const main2 = options.getMainWindow();
      if (main2 && !main2.isDestroyed()) {
        main2.webContents.send("system-video-window:closed", lastState);
      }
    });
  ...
  function updateState(payload) {
    ...
    const main2 = options.getMainWindow();
    if (main2 && !main2.isDestroyed()) {
      main2.webContents.send("system-video-window:state", payload);
    }
  }
  function destroy() {
    if (videoWin && !videoWin.isDestroyed()) {
...
function registerAppUpdateIpcHandlers(ipcMain2, service) {
  ipcMain2.handle("app-update:get-state", () => service.getState());
  ipcMain2.handle("app-update:check", () => service.check());
  ipcMain2.handle("app-update:download", () => service.download());
  ipcMain2.handle("app-update:install", () => service.install());
}
var main$1 = {};
var fs$i = {};
var universalify$1 = {};
...
`));
  ...
  static reconstructOriginalUrl(options) {
    ...
    return new url_1$7.URL(`${protocol2}//${hostname}${port}${path2}`);
  }
...
class BaseUpdater extends AppUpdater_1$1.AppUpdater {
  ...
  spawnSyncLog(cmd, args = [], env = {}) {
    ...
    const response = (0, child_process_1$3.spawnSync)(cmd, args, {
      env: { ...mergedEnv, PATH: this.sanitizeEnvPath((_a2 = mergedEnv.PATH) !== null && _a2 !== void 0 ? _a2 : "") },
      encoding: "utf-8",
      shell: true
    });
...
const LOOPBACK_HOSTS = /* @__PURE__ */ new Set(["127.0.0.1", "::1", "localhost"]);
function normalizeAppUpdateBaseUrl(input) {
...
const AUTO_CHECK_DELAY_MS = 15e3;
const AUTO_CHECK_INTERVAL_MS = 6 * 60 * 60 * 1e3;
const UPDATE_LOG_FILENAME = "app-update.log";
const MAX_UPDATE_LOG_BYTES = 1024 * 1024;
const MAC_UPDATE_VALIDATION_SETTLE_MS = 3e3;
function normalizeReleaseNotes(info) {
  ...
  const notes = releaseNotes.map((item) => String((item == null ? void 0 : item.note) || "").trim()).filter(Boolean).join("\n\n");
  return notes || null;
}
function toErrorMessage(error2) {
  if (error2 instanceof Error && error2.message) return error2.message;
  return String(error2 || "未知更新错误");
}
function toUserErrorMessage(error2) {
...
class AppUpdateService {
  constructor(options) {
    __publicField(this, "getMainWindow");
    __publicField(this, "updateBaseUrl");
    __publicField(this, "snapshot");
    __publicField(this, "initialized", false);
    __publicField(this, "autoCheckTimer", null);
    __publicField(this, "autoCheckInterval", null);
    __publicField(this, "activeOperation", null);
    __publicField(this, "validationTimer", null);
    __publicField(this, "handleCheckingForUpdate", () => {
      this.patchState({ message: null, status: "checking" });
    });
    __publicField(this, "handleUpdateAvailable", (info) => {
    ...
    __publicField(this, "handleUpdateNotAvailable", () => {
    ...
    __publicField(this, "handleDownloadProgress", (progress) => {
      this.patchState({
        message: null,
        progress: {
    ...
    __publicField(this, "handleUpdateDownloaded", (info) => {
    ...
    __publicField(this, "handleError", (error2) => {
    ...
    const message = !platformSupported ? "当前仅在 macOS 启用应用内更新" : !app.isPackaged ? "开发模式不执行应用内更新" : !this.updateBaseUrl ? "当前安装包未配置更新地址" : null;
    this.snapshot = {
      availableVersion: null,
      checkedAt: null,
  ...
  initialize() {
    ...
    main$1.autoUpdater.autoRunAppAfterInstall = true;
    ...
    main$1.autoUpdater.on("checking-for-update", this.handleCheckingForUpdate);
    main$1.autoUpdater.on("update-available", this.handleUpdateAvailable);
    main$1.autoUpdater.on("update-not-available", this.handleUpdateNotAvailable);
    main$1.autoUpdater.on("download-progress", this.handleDownloadProgress);
    main$1.autoUpdater.on("update-downloaded", this.handleUpdateDownloaded);
    main$1.autoUpdater.on("error", this.handleError);
  ...
  dispose() {
    ...
    main$1.autoUpdater.removeListener("checking-for-update", this.handleCheckingForUpdate);
    main$1.autoUpdater.removeListener("update-available", this.handleUpdateAvailable);
    main$1.autoUpdater.removeListener("update-not-available", this.handleUpdateNotAvailable);
    main$1.autoUpdater.removeListener("download-progress", this.handleDownloadProgress);
    main$1.autoUpdater.removeListener("update-downloaded", this.handleUpdateDownloaded);
    main$1.autoUpdater.removeListener("error", this.handleError);
  ...
  check() {
    return this.runOperation(async () => {
      ...
      if (this.snapshot.status === "downloading" || this.snapshot.status === "verifying" || this.snapshot.status === "downloaded") {
  ...
  runOperation(operation) {
    ...
    const promise = operation().catch((error2) => {
      this.setError(error2);
      return this.getState();
    }).finally(() => {
  ...
  patchState(patch2) {
    this.snapshot = { ...this.snapshot, ...patch2 };
    const mainWindow2 = this.getMainWindow();
    if (mainWindow2 && !mainWindow2.isDestroyed() && !mainWindow2.webContents.isDestroyed()) {
      mainWindow2.webContents.send(APP_UPDATE_STATE_CHANNEL, this.getState());
    }
  }
  setError(error2) {
    ...
    this.writeLog("error", toErrorMessage(error2));
    this.patchState({ message: toUserErrorMessage(error2), status: "error" });
  }
  writeLog(level, message) {
    try {
      const logPath = path$n.join(app.getPath("userData"), UPDATE_LOG_FILENAME);
      if (existsSync(logPath) && statSync(logPath).size > MAX_UPDATE_LOG_BYTES) {
        truncateSync(logPath, 0);
      }
      appendFileSync(logPath, `${(/* @__PURE__ */ new Date()).toISOString()} [${level}] ${message}
`, "utf8");
    } catch {
    }
  }
...
function getMainWindowPlatformOptions(platform2 = process.platform) {
  if (platform2 === "darwin") {
...
try {
  const stableUserDataPath = path$n.join(app.getPath("appData"), resolveUserDataDirname());
...
function getWindowStateFilePath() {
  return path$n.join(app.getPath("userData"), WINDOW_STATE_FILENAME);
}
...
function readPersistedWindowState() {
  try {
    ...
    const raw = readFileSync$1(filePath, "utf-8");
    const parsed = JSON.parse(raw);
...
function saveWindowState(win) {
  ...
  try {
    ...
    const filePath = getWindowStateFilePath();
    mkdirSync(path$n.dirname(filePath), { recursive: true });
    writeFileSync$1(filePath, JSON.stringify(payload), "utf-8");
  } catch {
  }
}
...
const overlayWindowController = createOverlayWindowController({
  getMainWindow: () => mainWindow,
  preloadPath: path$n.join(MAIN_DIST, "preload.mjs"),
...
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    void win.loadFile(path$n.join(RENDERER_DIST, "index.html"));
  }
  return win;
}
app.on("before-quit", () => {
