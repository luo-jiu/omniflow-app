...
生成器只从指定 commit 的 Git 对象读取 contracts、inventory、schemas 和 tracked source，不会在 blob 缺失时回退到同名 worktree 文件。每份 blob bytes 都会按仓库 object format 重新计算 Git object id 并绑定 tree OID；路径按 literal pathspec 读取，nested historical/deletion commit 也必须是可用的完整 commit object id，不能用相对 revision 或 annotated tag object 代替。raw bytes 验证成功的 commit metadata 使用按仓库和完整 object id 绑定的有界 LRU 复用，并向调用方返回 defensive copy；失败结果不缓存，branch/tag/HEAD 等可移动 revision 每次先重新解析，缓存命中也不会减免 scanner 的逻辑预算计费。`sourceManifest` 覆盖该 commit 的完整 tracked blob tree，并按原始 bytes 记录 mode、长度和 SHA-256；`docs/cat-catch/report-index/**` 是防止证据哈希递归的唯一固定排除项。exact-commit profile 仍加载全部 report-index schemas 供引用编译，但完全不读取 `report-index/index.json` 数据；删除或损坏派生索引不能改变 closure 是否可生成。生成器和 CLI 也不会写入或更新该目录。

所有 inventory current-node 的 path、source hash、capability/cutover 映射和带 symbol 的 AST locator 都会在该 commit 的 blob 上重新验证。JSON、Git path 和源码必须能无损解码为 UTF-8；非法 bytes 不会先替换成 U+FFFD 再继续。所有 source hash 与 locator 均已验证的 current-node 都进入 `discoveredNodes`：bootstrap root 标记为 `reachable`，其余节点明确标记为 `unknown`。missing、ambiguous、parse-error、unsupported-language 和非法编码都 fail closed；null capability/cutover pair 作为合法但未映射的 gap 进入 `unmappedInScopeNodes`。

当前 validator 已提供可独立验证的 exact-commit discovery 原语：AST static import/export/re-export/call/construct/窄义 dependency-injection 图与反向索引、按组匹配并区分 candidate/audit-reference/generated-mirror 的 semantic scan、带类型 query 与精确 Git range 的 historical touchset scan，以及带轮次证据的方向保持 least-fixed-point closure。这些原语还没有由 local-closure generator 组合并投影到当前 report；即使没有 fatal diagnostic，static graph 的 call、dependency-injection 和总覆盖仍明确为 `partial`。未解析边尚未与 declared dynamic edge 对账，`dist-electron` 等 generated mirror 也尚未建立绑定 source/build inputs 的排除合同。因此对应 `discoveryCoverage` 维度仍保持 `pending`，每个非 root 节点继续保留 `closure.current-node-reachability-undetermined` blocker；原语存在不代表 local closure 已经 complete。

四类 discovery 原语都有可执行、默认有界且测试可覆盖的 fail-closed 预算。Semantic scan 约束 path、input bytes、rule、pattern、literal search work 和 hit，拒绝非 canonical blob path，并在 hit 超限时清空 partial matches；static graph 在 snapshot/hash/UTF-8/TypeScript program 与 AST 创建前约束 blob/manifest entries、总 blob bytes、supported source files/bytes 和 inventory locators；closure 在遍历前约束 node/edge/seed，并对 runtime 与 scope 共用总 round/transition-inspection 预算，任一超限都不返回 partial graph。Historical scan 对 ancestry commits、raw commit bytes、recursive tree entries、unique blob objects/bytes、compared paths/changes、searched source bytes 和 exact hits 设置整次 validation 固定总额；各 touchset 从总额等分预算，commit-message/blob 的总搜索工作量在 literal scan 和 blob decode 前预检。任何原语超限都不能以不完整结果继续声称 exact coverage。

Schema v2 对 inventory 做无损投影：bootstrap root 保留 id/category/traversal/typed locator；current-node 保留 classification 与逐节点 provenance；declared dynamic edge 保留 source/target、fixture、resolution rule 与 hash；historical candidate 保留 `touchsetId`、typed `discoveryEvidence` 与 `lastKnownCommit`，current candidate 对前两项显式投影为 `null`；approved exclusion、retired tombstone 和 external-process endpoint 保留完整归属与证据字段。Declared dynamic edge 只有在 typed source/target locator 和 exact-commit source hash 均可验证时才进入 `declaredDynamicEdges`；无法验证的合法声明进入包含完整声明/实际 hash 的 `unresolvedDynamicEdges`。`discoveryCoverage.declaredDynamicEdges=complete` 只表示全部声明都已遍历和校验，不表示结果全部已解析，任何 unresolved edge 仍会阻止 closure。`process-handoff` 的虚拟端点单列在 `externalProcessEndpoints`，每条 attribution 逐 edge 绑定 source node、capability、cutover unit、source hash 和来源 reachability，不混入 current source node，也不使用会产生笛卡尔歧义的独立集合。approved exclusion 必须反向唯一绑定真实 semantic/historical candidate，historical source 还会在 `lastKnownCommit` 的 Git blob 上验证 path、hash 和 locator；retired tombstone 会验证 deletion commit 是当前输入的祖先、删除前 parent bytes/hash/locator、删除提交触及目标路径，并证明该 locator 在删除提交和当前输入中均已不存在。

生成投影前会执行只读 exact-commit 跨文件 invariant。重复 id/typed locator、未知 capability/cutover 引用、ledger legacy ownerRef 归属错误、historical resolution 错配、dynamic fixture/endpoint/sourceHash 自相矛盾等结构错误不会降级成普通 blocker，而是拒绝生成报告并返回状态码 `2`；同一物理 historical query hit 即使由不同 query 或 touchset 再次声明，也只能绑定一个 candidate。该检查只读取所选 commit 的 ledger/inventory Git blobs，不读取 worktree、HEAD 或 report-index。声明结构自洽但 source blob 已变化、locator 暂时不可验证或 discovery coverage 尚未完成时，才会生成可审计的 `blocked` 候选。
